...
#define MAX_NUM_RING_SOCKETS          128
...
/* Versioning */
#define RING_VERSION                "6.3.0"
#define RING_VERSION_NUM           0x060300
...
typedef struct __pf_t_buf
{
    struct task_struct *task;
    pf_t_data *pf_tb_data;
    uint32_t total_cnt;
    uint32_t drop_cnt;
    uint32_t sleep_cnt;
    uint32_t sleep_cnt_r;
    atomic_t coms;
    atomic_t proc;
    atomic_t cnt;
    unsigned int acid;
    uint8_t tid;
    uint8_t align_id;
    volatile int8_t state;  //1, running, 0, wait for waking up, -1, exited
    volatile int8_t dev_pack_state;
    volatile uint8_t buf_alive;	//1, in memory, 0, released
    //rwlock_t disp_lock;
    spinlock_t disp_lock;
}pf_t_buf;

#define PF_T_MASK			(0xFFFF)
#define PF_T_SLOT			(PF_T_MASK+1)//(49152)//(98304)//(393216)//(196608)//(min_num_slots)
#define PF_T_BUF_LEN		((sizeof(pf_t_data)*PF_T_SLOT))
