...
volatile u_int8_t pfring_enabled = 1;
volatile u_int8_t pfring_dev_pack_run = 0;
...
static struct packet_type prot_hook_2;
...
static struct pf_ring_socket* device_rings[MAX_NUM_IFIDX][MAX_NUM_RX_CHANNELS] =
        { { NULL } };
...
static int reflect_packet(struct sk_buff *skb, struct pf_ring_socket *pfr,
        struct net_device *reflector_dev, int displ,
        rule_action_behaviour behaviour, u_int8_t do_clone_skb);
...
static int skb_ring_handler(struct sk_buff *skb, u_char recv_packet,
        u_int8_t real_skb, u_int8_t *skb_reference_in_use, int32_t channel_id,
...
static int get_fragment_app_id(u_int32_t ipv4_src_host, u_int32_t ipv4_dst_host,
        u_int16_t fragment_id, u_int8_t more_fragments);
static void add_fragment_app_id(u_int32_t ipv4_src_host,
        u_int32_t ipv4_dst_host, u_int16_t fragment_id, u_int8_t app_id);
...
MODULE_PARM_DESC(transparent_mode, "(deprecated)");
MODULE_PARM_DESC(enable_debug,
        "Set to 1 to enable PF_RING debug tracing into the syslog");
MODULE_PARM_DESC(enable_tx_capture, "Set to 1 to capture outgoing packets");
MODULE_PARM_DESC(enable_frag_coherence,
        "Set to 1 to handle fragments (flow coherence) in clusters");
MODULE_PARM_DESC(enable_ip_defrag,
        "Set to 1 to enable IP defragmentation" "(only rx traffic is defragmentead)");
...
int lockless_list_remove(lockless_list *l, void *elem)
{
    ...
    for (i = 0; i < MAX_NUM_LIST_ELEMENTS; i++) {
        if (l->list_elements[i] == elem) {
            ...
            while ((l->top_element_id > 0)
                    && (l->list_elements[l->top_element_id] == NULL))
                l->top_element_id--;
...
static inline u_int64_t get_next_slot_offset(struct pf_ring_socket *pfr,
        u_int64_t off)
{
    ...
    if ((off + real_slot_size + pfr->slots_info->slot_len)
            > (pfr->slots_info->tot_mem - sizeof(FlowSlotInfo)))
        return 0;
    ...
    while (pfr->slots_info->kernel_remove_off != pfr->slots_info->remove_off
            ...
            (synchronized
                    || pfr->slots_info->remove_off
                            != get_next_slot_offset(pfr,
                                    pfr->slots_info->kernel_remove_off))) {
        struct pfring_pkthdr *hdr =
                (struct pfring_pkthdr*) &pfr->ring_slots[pfr->slots_info->kernel_remove_off];

        if (unlikely(enable_debug))
            printk(
                    "[PF_RING] Original offset [kernel_remove_off=%llu][remove_off=%llu][skb=%p]\n",
                    pfr->slots_info->kernel_remove_off,
                    pfr->slots_info->remove_off, hdr->extended_hdr.tx.reserved);

        if (hdr->extended_hdr.tx.reserved != NULL) {
            /* Can't forward the packet on the same interface it has been received */
            if (hdr->extended_hdr.tx.bounce_interface
                    == pfr->ring_netdev->dev->ifindex) {
            ...
            if (hdr->extended_hdr.tx.bounce_interface != UNKNOWN_INTERFACE) {
                ...
                if (pfr->tx.last_tx_dev_idx
                    ...
                    /* Reset all */
                    pfr->tx.last_tx_dev = NULL, pfr->tx.last_tx_dev_idx =
                    UNKNOWN_INTERFACE;
        ...
        pfr->slots_info->kernel_remove_off = get_next_slot_offset(pfr,
                pfr->slots_info->kernel_remove_off);
        pfr->slots_info->kernel_tot_read++;

        if (unlikely(enable_debug))
            printk(
                    "[PF_RING] New offset [kernel_remove_off=%llu][remove_off=%llu]\n",
                    pfr->slots_info->kernel_remove_off,
                    pfr->slots_info->remove_off);
    }
}
...
static const struct file_operations ring_proc_fops = { .owner = THIS_MODULE,
        .open = ring_proc_open, .read = seq_read, .llseek = seq_lseek,
...
static void ring_proc_add(struct pf_ring_socket *pfr)
{
    ...
    if ((ring_proc_dir != NULL) && (pfr->sock_proc_name[0] == '\0')) {
        snprintf(pfr->sock_proc_name, sizeof(pfr->sock_proc_name), "%d-%s.%d",
                pfr->ring_pid, pfr->ring_netdev->dev->name, pfr->ring_id);

        proc_create_data(pfr->sock_proc_name, 0, ring_proc_dir, &ring_proc_fops,
                pfr);
    ...
    if (m->private != NULL) {
        ...
        if (dev_ptr->is_zc_device) {
            ...
            case standard_nic_family:
                dev_family = "Standard NIC";
                break;
            case intel_82599_family:
                dev_family = "Intel 82599";
                break;
            }
        }
        ...
        switch (dev->type) {
        case 1:
            strcpy(dev_buf, "Ethernet");
            break;
        case 772:
            strcpy(dev_buf, "Loopback");
            break;
        default:
            sprintf(dev_buf, "%d", dev->type);
            break;
        }
        ...
        seq_printf(m, "# Used RX Queues:  %d\n",
                dev_ptr->is_zc_device ?
                        dev_ptr->num_zc_dev_rx_queues : get_num_rx_queues(dev));
        ...
        hw_filtering_rule *rule, hw_filtering_rule_command request)
{
    ...
    if ((dev->ethtool_ops == NULL) || (dev->ethtool_ops->set_rxnfc == NULL))
        return (-1);

    if (unlikely(enable_debug))
        printk("[PF_RING] hw_filtering_rule[%s][request=%d][%p]\n", dev->name,
                request, dev->ethtool_ops->set_rxnfc);
    ...
    case intel_82599_five_tuple_rule:
        ...
        cmd.cmd = (request == add_hw_rule ?
        ETHTOOL_PFRING_SRXFTRLINS :
                                            ETHTOOL_PFRING_SRXFTRLDEL);
    ...
    if (cmd.cmd) {
        ...
        if (unlikely(enable_debug)
                ...
                && rc < 0) {
            intel_82599_perfect_filter_hw_rule *perfect_rule =
                    &rule->rule_family.perfect_rule;

            printk(
                    "[DEBUG] %s() ixgbe_set_rxnfc(%d.%d.%d.%d:%d -> %d.%d.%d.%d:%d) returned %d\n",
                    __FUNCTION__, perfect_rule->s_addr >> 24 & 0xFF,
                    perfect_rule->s_addr >> 16 & 0xFF,
                    perfect_rule->s_addr >> 8 & 0xFF,
                    perfect_rule->s_addr >> 0 & 0xFF,
                    perfect_rule->s_port & 0xFFFF,
                    perfect_rule->d_addr >> 24 & 0xFF,
                    perfect_rule->d_addr >> 16 & 0xFF,
                    perfect_rule->d_addr >> 8 & 0xFF,
                    perfect_rule->d_addr >> 0 & 0xFF,
                    perfect_rule->d_port & 0xFFFF, rc);
        }
    }
#endif
    return (rc);
...
static int handle_hw_filtering_rule(struct pf_ring_socket *pfr,
        hw_filtering_rule *rule, hw_filtering_rule_command command)
{
    ...
    switch (rule->rule_family_type) {
    case intel_82599_five_tuple_rule:
        if (pfr->ring_netdev->hw_filters.filter_handlers.five_tuple_handler
                == NULL)
            return (-EINVAL);
        else
    ...
    case intel_82599_perfect_filter_rule:
        if (pfr->ring_netdev->hw_filters.filter_handlers.perfect_filter_handler
                == NULL)
            return (-EINVAL);
        else
            return (i82599_generic_handler(pfr, rule, command));
...
static int ring_proc_get_info(struct seq_file *m, void *data_not_used)
{
    ...
    if (m->private == NULL) {
        ...
        seq_printf(m, "RX cnt:                  : %u, %u, %u, %u\n",
        		pf_td_un[0].total_cnt, pf_td_un[1].total_cnt, pf_td_un[2].total_cnt, pf_td_un[3].total_cnt);
        seq_printf(m, "DROP cnt:                : %u, %u, %u, %u\n",
        		pf_td_un[0].drop_cnt, pf_td_un[1].drop_cnt, pf_td_un[2].drop_cnt, pf_td_un[3].drop_cnt);
        seq_printf(m, "SLEEP cnt:                : %u, %u, %u, %u\n",
        		pf_td_un[0].sleep_cnt, pf_td_un[1].sleep_cnt, pf_td_un[2].sleep_cnt, pf_td_un[3].sleep_cnt);
        seq_printf(m, "Real SLEEP cnt:           : %u, %u, %u, %u\n",
        		pf_td_un[0].sleep_cnt_r, pf_td_un[1].sleep_cnt_r, pf_td_un[2].sleep_cnt_r, pf_td_un[3].sleep_cnt_r);
        ...
        if (pfr) {
            ...
            if (pfr->custom_bound_device_name[0] != '\0') {
                ...
                {
                    ring_device_element *dev_ptr = list_entry(ptr,
                            ring_device_element, device_list);

                    if (test_bit(dev_ptr->dev->ifindex, pfr->netdev_mask)) {
                        seq_printf(m, "%s%s", (num > 0) ? "," : "",
                                dev_ptr->dev->name);
                        num++;
                    }
                }
            ...
            seq_printf(m, "Breed              : %s\n",
                    (pfr->zc_device_entry != NULL) ?
                            (pfr->ring_netdev->is_zc_device == 1 ? "DNA" : "ZC") :
                            "Standard");
            ...
            if (pfr->zc_device_entry != NULL) {
                ...
                if (pfr->dna_cluster && pfr->dna_cluster_type == cluster_master
                        && pfr->dna_cluster->stats) {
...
static int ring_proc_plugins_open(struct inode *inode, struct file *file)
{
    return single_open(file, ring_proc_get_plugin_info, PDE_DATA(inode));
}

static const struct file_operations ring_proc_plugins_fops = { .owner =
THIS_MODULE, .open = ring_proc_plugins_open, .read = seq_read, .llseek =
...
static void ring_proc_init(void)
{
    ...
    if (ring_proc_dir != NULL) {
        ...
        ring_proc = proc_create(PROC_INFO /* name */, 0 /* read-only */,
                ring_proc_dir /* parent */,
                &ring_proc_fops /* file operations */);

        ring_proc_plugins_info = proc_create(PROC_PLUGINS_INFO,
                0 /* read-only */, ring_proc_dir, &ring_proc_plugins_fops);
...
static int ring_alloc_mem(struct sock *sk)
{
    ...
    the_slot_len = ALIGN(
            the_slot_len + sizeof(u_int16_t) /* RING_MAGIC_VALUE */,
            sizeof(u_int64_t));

    if (unlikely(
            (UINT_MAX - sizeof(FlowSlotInfo)) / the_slot_len < min_num_slots)) {
        printk(
                "[PF_RING] ERROR: min_num_slots (%u, slot len = %u) causes memory size to wrap\n",
    ...
    pfr->num_sw_filtering_hash = pfr->num_sw_filtering_rules =
            pfr->num_hw_filtering_rules = 0;
...
static inline int ring_insert(struct sock *sk)
{
    ...
    bitmap_zero(pfr->netdev_mask, MAX_NUM_DEVICES_ID), pfr->num_bound_devices =
            0;
...
static inline void ring_remove(struct sock *sk_to_delete)
{
    ...
    while (sk != NULL) {
        ...
        if (master_found && socket_found)
            ...
            sk = (struct sock*) lockless_list_get_next(&ring_table,
                    &last_list_idx);
    }
...
static inline u_int32_t hash_pkt(u_int16_t vlan_id, u_int8_t proto,
        ip_addr host_peer_a, ip_addr host_peer_b, u_int16_t port_peer_a,
        u_int16_t port_peer_b)
{
    ...
    return (vlan_id + proto + host_peer_a.v6.s6_addr32[0]
            + host_peer_a.v6.s6_addr32[1] + host_peer_a.v6.s6_addr32[2]
            + host_peer_a.v6.s6_addr32[3] + host_peer_b.v6.s6_addr32[0]
            + host_peer_b.v6.s6_addr32[1] + host_peer_b.v6.s6_addr32[2]
            + host_peer_b.v6.s6_addr32[3] + port_peer_a + port_peer_b);
}
...
static inline int parse_raw_pkt_ip(u_char *data, u_int data_len,
        struct pfring_pkthdr *hdr, u_int16_t *ip_id)
{
    struct ethhdr *eh = (struct ethhdr *) data;
    u_int16_t displ = sizeof(struct ethhdr), ip_len, fragment_offset = 0;
//            tunnel_offset = 0;

    memset(&hdr->extended_hdr.parsed_pkt, 0,
    ...
    *ip_id = 0;
    ...
    memcpy(&hdr->extended_hdr.parsed_pkt.smac, eh->h_source,
            sizeof(eh->h_source));
    ...
    if (hdr->extended_hdr.parsed_pkt.eth_type == ETH_P_IP /* IPv4 */) {
        ...
        if (data_len < hdr->extended_hdr.parsed_pkt.offset.l3_offset
                        + sizeof(struct iphdr))
            return (0);
        ...
        *ip_id = ip->id, frag_off = ntohs(ip->frag_off);
        ...
        if (data_len < hdr->extended_hdr.parsed_pkt.offset.l3_offset
                        + sizeof(struct kcompact_ipv6_hdr))
            return (0);
        ...
        memcpy(&hdr->extended_hdr.parsed_pkt.ip_src.v6, &ipv6->saddr,
                sizeof(ipv6->saddr));
        memcpy(&hdr->extended_hdr.parsed_pkt.ip_dst.v6, &ipv6->daddr,
                sizeof(ipv6->daddr));
        ...
        while (hdr->extended_hdr.parsed_pkt.l3_proto == NEXTHDR_HOP
            ...
            if (data_len < hdr->extended_hdr.parsed_pkt.offset.l3_offset + ip_len
                            + sizeof(struct kcompact_ipv6_opt_hdr))
                return 1;

            ipv6_opt = (struct kcompact_ipv6_opt_hdr *) (&data[hdr->extended_hdr.parsed_pkt.offset.l3_offset
                            + ip_len]);
            ip_len += sizeof(struct kcompact_ipv6_opt_hdr);
            if (hdr->extended_hdr.parsed_pkt.l3_proto == NEXTHDR_HOP
                    || hdr->extended_hdr.parsed_pkt.l3_proto == NEXTHDR_DEST
                    || hdr->extended_hdr.parsed_pkt.l3_proto == NEXTHDR_ROUTING)
                ip_len += (ipv6_opt->hdrlen << 3);
...
static int parse_pkt_ip(struct sk_buff *skb, u_int8_t real_skb, int skb_displ,
        struct pfring_pkthdr *hdr, u_int16_t *ip_id)
{
    int rc;
    u_char buffer[128]; /* Enough for standard and tunneled headers */
    int data_len = min((u_int16_t) (skb->len + skb_displ),
            (u_int16_t) sizeof(buffer));

    skb_copy_bits(skb, -skb_displ, buffer, data_len);

    rc = parse_raw_pkt_ip(buffer, data_len, hdr, ip_id);
        ...
        struct pfring_pkthdr *hdr, u_int16_t *ip_id)
{
    ...
    *ip_id = 0;
    ...
    if (hdr->extended_hdr.parsed_pkt.eth_type == ETH_P_8021Q /* 802.1q (VLAN) */) {
        ...
        && displ <= data_len) {
            hdr->extended_hdr.parsed_pkt.offset.vlan_offset +=
                    sizeof(struct eth_vlan_hdr);
            vh =
                    (struct eth_vlan_hdr *) &data[hdr->extended_hdr.parsed_pkt.offset.vlan_offset];
    ...
    if (hdr->extended_hdr.parsed_pkt.eth_type == ETH_P_IP /* IPv4 */) {
        ...
        ip =
                (struct iphdr *) (&data[hdr->extended_hdr.parsed_pkt.offset.l3_offset]);
        *ip_id = ip->id, frag_off = ntohs(ip->frag_off);
        ...
        ipv6 =
                (struct kcompact_ipv6_hdr*) (&data[hdr->extended_hdr.parsed_pkt.offset.l3_offset]);
        ip_len = sizeof(struct kcompact_ipv6_hdr);
        ...
        while (hdr->extended_hdr.parsed_pkt.l3_proto == NEXTHDR_HOP
            ...
            if (data_len
                    < hdr->extended_hdr.parsed_pkt.offset.l3_offset + ip_len
                            + sizeof(struct kcompact_ipv6_opt_hdr))
                return 1;

            ipv6_opt =
                    (struct kcompact_ipv6_opt_hdr *) (&data[hdr->extended_hdr.parsed_pkt.offset.l3_offset
                            + ip_len]);
            ip_len += sizeof(struct kcompact_ipv6_opt_hdr);
            if (hdr->extended_hdr.parsed_pkt.l3_proto == NEXTHDR_HOP
                    || hdr->extended_hdr.parsed_pkt.l3_proto == NEXTHDR_DEST
    ...
    if (((hdr->extended_hdr.parsed_pkt.l3_proto == IPPROTO_TCP)
        ...
        if (hdr->extended_hdr.parsed_pkt.l3_proto == IPPROTO_TCP) {
            ...
            if (data_len
                ...
                return (1);
            tcp =
                    (struct tcphdr *) (&data[hdr->extended_hdr.parsed_pkt.offset.l4_offset]);
            ...
            hdr->extended_hdr.parsed_pkt.tcp.flags = (tcp->fin
                    * TH_FIN_MULTIPLIER) + (tcp->syn * TH_SYN_MULTIPLIER)
            ...
            if (data_len
                ...
                return (1);
            udp =
                    (struct udphdr *) (&data[hdr->extended_hdr.parsed_pkt.offset.l4_offset]);

            hdr->extended_hdr.parsed_pkt.l4_src_port = ntohs(udp->source), hdr->extended_hdr.parsed_pkt.l4_dst_port =
                    ntohs(udp->dest);
            hdr->extended_hdr.parsed_pkt.offset.payload_offset =
                    hdr->extended_hdr.parsed_pkt.offset.l4_offset
                            + sizeof(struct udphdr);
            ...
            if ((hdr->extended_hdr.parsed_pkt.l4_src_port == GTP_SIGNALING_PORT)
                ...
                if (data_len
                        < (hdr->extended_hdr.parsed_pkt.offset.payload_offset
                                + sizeof(struct gtp_v1_hdr)))
                    return (1);

                gtp =
                        (struct gtp_v1_hdr *) (&data[hdr->extended_hdr.parsed_pkt.offset.payload_offset]);
                gtp_len = sizeof(struct gtp_v1_hdr);

                if (((gtp->flags & GTP_FLAGS_VERSION) >> GTP_FLAGS_VERSION_SHIFT)
                        == GTP_VERSION_1) {
                    struct iphdr *tunneled_ip;

                    hdr->extended_hdr.parsed_pkt.tunnel.tunnel_id = ntohl(
                            gtp->teid);

                    //if(unlikely(enable_debug))
                    //  printk("[PF_RING] GTPv1 [%04X]\n", hdr->extended_hdr.parsed_pkt.tunnel.tunnel_id);

                    if ((hdr->extended_hdr.parsed_pkt.l4_src_port
                            == GTP_U_DATA_PORT)
                            || (hdr->extended_hdr.parsed_pkt.l4_dst_port
                                    == GTP_U_DATA_PORT)) {
                        if (gtp->flags
                                & (GTP_FLAGS_EXTENSION | GTP_FLAGS_SEQ_NUM
                                        | GTP_FLAGS_NPDU_NUM)) {
                            ...
                            if (data_len
                                    < (hdr->extended_hdr.parsed_pkt.offset.payload_offset
                                            + gtp_len
                                            + sizeof(struct gtp_v1_opt_hdr)))
                                return (1);

                            gtpopt =
                                    (struct gtp_v1_opt_hdr *) (&data[hdr->extended_hdr.parsed_pkt.offset.payload_offset
                                            + gtp_len]);
                            gtp_len += sizeof(struct gtp_v1_opt_hdr);

                            if ((gtp->flags & GTP_FLAGS_EXTENSION)
                                    && gtpopt->next_ext_hdr) {
                                ...
                                do {
                                    if (data_len
                                            < (hdr->extended_hdr.parsed_pkt.offset.payload_offset
                                                    + gtp_len + 1 /* 8 bit len field */))
                                        return (1);
                                    gtpext =
                                            (struct gtp_v1_ext_hdr *) (&data[hdr->extended_hdr.parsed_pkt.offset.payload_offset
                                                    + gtp_len]);
                                    gtp_len += (gtpext->len
                                    ...
                                    if ((gtpext->len == 0)
                                        ...
                                        return (1);
                                    next_ext_hdr =
                                            (u_int8_t *) (&data[hdr->extended_hdr.parsed_pkt.offset.payload_offset
                                                    + gtp_len - 1 /* 8 bit next_ext_hdr field */]);
                        ...
                        if (data_len
                            ...
                            return (1);
                        tunneled_ip =
                                (struct iphdr *) (&data[hdr->extended_hdr.parsed_pkt.offset.payload_offset
                                        + gtp_len]);

                        if (tunneled_ip->version == 4 /* IPv4 */) {
                            hdr->extended_hdr.parsed_pkt.tunnel.tunneled_proto =
                                    tunneled_ip->protocol;
                            hdr->extended_hdr.parsed_pkt.tunnel.tunneled_ip_src.v4 =
                                    ntohl(tunneled_ip->saddr);
                            hdr->extended_hdr.parsed_pkt.tunnel.tunneled_ip_dst.v4 =
                                    ntohl(tunneled_ip->daddr);
                            fragment_offset = tunneled_ip->frag_off
                                    & htons(IP_OFFSET); /* fragment, but not the first */
                            ...
                            if (data_len
                                ...
                                return (1);
                            tunneled_ipv6 =
                                    (struct kcompact_ipv6_hdr *) (&data[hdr->extended_hdr.parsed_pkt.offset.payload_offset
                                            + gtp_len]);

                            hdr->extended_hdr.parsed_pkt.tunnel.tunneled_proto =
                                    tunneled_ipv6->nexthdr;
                            /* Values of IPv6 addresses are stored as network byte order */
                            memcpy(
                                    &hdr->extended_hdr.parsed_pkt.tunnel.tunneled_ip_src.v6,
                                    &tunneled_ipv6->saddr,
                                    sizeof(tunneled_ipv6->saddr));
                            memcpy(
                                    &hdr->extended_hdr.parsed_pkt.tunnel.tunneled_ip_dst.v6,
                                    &tunneled_ipv6->daddr,
                                    sizeof(tunneled_ipv6->daddr));

                            ip_len = sizeof(struct kcompact_ipv6_hdr), tunnel_offset =
                                    hdr->extended_hdr.parsed_pkt.offset.payload_offset
                                            + gtp_len;

                            while (hdr->extended_hdr.parsed_pkt.tunnel.tunneled_proto
                                    == NEXTHDR_HOP
                                    || hdr->extended_hdr.parsed_pkt.tunnel.tunneled_proto
                                            == NEXTHDR_DEST
                                    || hdr->extended_hdr.parsed_pkt.tunnel.tunneled_proto
                                ...
                                ipv6_opt =
                                        (struct kcompact_ipv6_opt_hdr *) (&data[tunnel_offset
                                                + ip_len]);
                                ip_len += sizeof(struct kcompact_ipv6_opt_hdr), fragment_offset =
                                        0;
                                if (hdr->extended_hdr.parsed_pkt.tunnel.tunneled_proto
                                        == NEXTHDR_AUTH)
                                    /*
                                ...
                                else if (hdr->extended_hdr.parsed_pkt.tunnel.tunneled_proto
                                        != NEXTHDR_FRAGMENT)
                        ...
                        parse_tunneled_packet: if (!fragment_offset) {
                            if (hdr->extended_hdr.parsed_pkt.tunnel.tunneled_proto
                                    == IPPROTO_TCP) {
                                struct tcphdr *tcp;

                                if (data_len
                                        < tunnel_offset + sizeof(struct tcphdr))
                                    return (1);
                                tcp = (struct tcphdr *) (&data[tunnel_offset]);

                                hdr->extended_hdr.parsed_pkt.tunnel.tunneled_l4_src_port =
                                        ntohs(tcp->source), hdr->extended_hdr.parsed_pkt.tunnel.tunneled_l4_dst_port =
                                        ntohs(tcp->dest);
                            } else if (hdr->extended_hdr.parsed_pkt.tunnel.tunneled_proto
                                    == IPPROTO_UDP) {
                                struct udphdr *udp;
                                ...
                                if (data_len
                                        < tunnel_offset + sizeof(struct udphdr))
                                    return (1);
                                udp = (struct udphdr *) (&data[tunnel_offset]);

                                hdr->extended_hdr.parsed_pkt.tunnel.tunneled_l4_src_port =
                                        ntohs(udp->source), hdr->extended_hdr.parsed_pkt.tunnel.tunneled_l4_dst_port =
                                        ntohs(udp->dest);

                                if ((hdr->extended_hdr.parsed_pkt.tunnel.tunneled_l4_src_port
                                        == MOBILE_IP_PORT)
                                        || (hdr->extended_hdr.parsed_pkt.tunnel.tunneled_l4_dst_port
                                                == MOBILE_IP_PORT)) {
            ...
            } else if ((hdr->extended_hdr.parsed_pkt.l4_src_port
                    == MOBILE_IP_PORT)
                    || (hdr->extended_hdr.parsed_pkt.l4_dst_port
                            == MOBILE_IP_PORT)) {
                /* FIX: missing implementation (TODO) */
            ...
            struct gre_header *gre =
                    (struct gre_header*) (&data[hdr->extended_hdr.parsed_pkt.offset.l4_offset]);
            int gre_offset;
            ...
            if ((gre->flags_and_version & GRE_HEADER_VERSION) == 0) {
                ...
                if (gre->flags_and_version & GRE_HEADER_KEY) {
                    u_int32_t *tunnel_id =
                            (u_int32_t*) (&data[hdr->extended_hdr.parsed_pkt.offset.l4_offset
                                    + gre_offset]);
                    gre_offset += 4;
                    hdr->extended_hdr.parsed_pkt.tunnel.tunnel_id = ntohl(
                            *tunnel_id);
                ...
                goto parse_tunneled_packet;
                /* Parse tunneled ports */
        ...
        hdr->extended_hdr.parsed_pkt.l4_src_port =
                hdr->extended_hdr.parsed_pkt.l4_dst_port = 0;
...
static int parse_pkt(struct sk_buff *skb, u_int8_t real_skb, int skb_displ,
        struct pfring_pkthdr *hdr, u_int16_t *ip_id)
{
    int rc;
    u_char buffer[128]; /* Enough for standard and tunneled headers */
...
static int hash_bucket_match(sw_filtering_hash_bucket *hash_bucket,
        struct pfring_pkthdr *hdr, u_char mask_src, u_char mask_dst)
{
#if 0
    ...
    if ((hash_bucket->rule.proto == hdr->extended_hdr.parsed_pkt.l3_proto)
            && (hash_bucket->rule.vlan_id
                    == hdr->extended_hdr.parsed_pkt.vlan_id)
            && (((hash_bucket->rule.host4_peer_a
                    == (mask_src ? 0 : hdr->extended_hdr.parsed_pkt.ipv4_src))
                    && (hash_bucket->rule.host4_peer_b
                            == (mask_dst ?
                                    0 : hdr->extended_hdr.parsed_pkt.ipv4_dst))
                    && (hash_bucket->rule.port_peer_a
                            == (mask_src ?
                                    0 : hdr->extended_hdr.parsed_pkt.l4_src_port))
                    && (hash_bucket->rule.port_peer_b
                            == (mask_dst ?
                                    0 : hdr->extended_hdr.parsed_pkt.l4_dst_port)))
                    || ((hash_bucket->rule.host4_peer_a
                            == (mask_dst ?
                                    0 : hdr->extended_hdr.parsed_pkt.ipv4_dst))
                            && (hash_bucket->rule.host4_peer_b
        ...
        if (hdr->extended_hdr.parsed_pkt.ip_version == 6) {
            if (((memcmp(&hash_bucket->rule.host6_peer_a,
                    (mask_src ?
                            &ip_zero.v6 : &hdr->extended_hdr.parsed_pkt.ipv6_src),
                    sizeof(ip_addr) == 0))
        ...
        hash_filtering_rule * rule)
{
    if (unlikely(enable_debug))
        printk("[PF_RING] (%u,%d,%d.%d.%d.%d:%u,%d.%d.%d.%d:%u) "
                ...
                ((hash_bucket->rule.host4_peer_b >> 8) & 0xff),
                ((hash_bucket->rule.host4_peer_b >> 0) & 0xff),
                hash_bucket->rule.port_peer_b, rule->vlan_id, rule->proto,
                ((rule->host4_peer_a >> 24) & 0xff),
                ((rule->host4_peer_a >> 16) & 0xff),
                ((rule->host4_peer_a >> 8) & 0xff),
                ((rule->host4_peer_a >> 0) & 0xff), rule->port_peer_a,
...
static inline int match_ipv6(ip_addr *addr, ip_addr *rule_addr,
        ip_addr *rule_mask)
{
    int i;
    if (rule_mask->v6.s6_addr32[0] != 0)
        for (i = 0; i < 4; i++)
            if ((addr->v6.s6_addr32[i] & rule_mask->v6.s6_addr32[i])
                    != rule_addr->v6.s6_addr32[i])
                return (0);
    return (1);
}
...
static int match_filtering_rule(struct pf_ring_socket *pfr,
        sw_filtering_rule_element * rule, struct pfring_pkthdr *hdr,
        struct sk_buff *skb, int displ,
        struct parse_buffer *parse_memory_buffer[], u_int8_t *free_parse_mem,
        u_int *last_matched_plugin, rule_action_behaviour *behaviour)
{
    ...
    *behaviour = rule->rule.rule_action;
    ...
    if ((rule->rule.core_fields.eth_type > 0)
            && (hdr->extended_hdr.parsed_pkt.eth_type
                    != rule->rule.core_fields.eth_type))
        return (0);
    ...
    if ((rule->rule.extended_fields.optional_fields & FILTER_TUNNEL_ID_FLAG)
            && (hdr->extended_hdr.parsed_pkt.tunnel.tunnel_id
                    != rule->rule.extended_fields.tunnel.tunnel_id))
        return (0);
    ...
    if (hdr->extended_hdr.parsed_pkt.ip_version == 6) {
        ...
        /* IPv4 */
        if ((hdr->extended_hdr.parsed_pkt.ip_src.v4
                & rule->rule.extended_fields.tunnel.dhost_mask.v4)
                != rule->rule.extended_fields.tunnel.dhost.v4
                || (hdr->extended_hdr.parsed_pkt.ip_dst.v4
                        & rule->rule.extended_fields.tunnel.shost_mask.v4)
                        != rule->rule.extended_fields.tunnel.shost.v4)
            return (0);
    }

    if ((memcmp(rule->rule.core_fields.dmac, empty_mac, ETH_ALEN) != 0)
            && (memcmp(hdr->extended_hdr.parsed_pkt.dmac,
                    rule->rule.core_fields.dmac, ETH_ALEN) != 0))
        goto swap_direction;

    if ((memcmp(rule->rule.core_fields.smac, empty_mac, ETH_ALEN) != 0)
            && (memcmp(hdr->extended_hdr.parsed_pkt.smac,
                    rule->rule.core_fields.smac, ETH_ALEN) != 0))
        goto swap_direction;
    ...
    if (hdr->extended_hdr.parsed_pkt.ip_version == 6) {
        ...
        /* IPv4 */
        if ((hdr->extended_hdr.parsed_pkt.ip_src.v4
                & rule->rule.core_fields.shost_mask.v4)
                != rule->rule.core_fields.shost.v4
                || (hdr->extended_hdr.parsed_pkt.ip_dst.v4
                        & rule->rule.core_fields.dhost_mask.v4)
    ...
    if ((rule->rule.core_fields.sport_high != 0)
            && ((hdr->extended_hdr.parsed_pkt.l4_src_port
                    < rule->rule.core_fields.sport_low)
                    || (hdr->extended_hdr.parsed_pkt.l4_src_port
                            > rule->rule.core_fields.sport_high)))
        goto swap_direction;

    if ((rule->rule.core_fields.dport_high != 0)
            && ((hdr->extended_hdr.parsed_pkt.l4_dst_port
                    < rule->rule.core_fields.dport_low)
                    || (hdr->extended_hdr.parsed_pkt.l4_dst_port
                            > rule->rule.core_fields.dport_high)))
        goto swap_direction;
    ...
    if (rule->rule.balance_pool > 0) {
        u_int32_t balance_hash = hash_pkt_header(hdr, 0)
                % rule->rule.balance_pool;
    ...
    if (rule->pattern[0] != NULL) {
        ...
        if ((hdr->extended_hdr.parsed_pkt.offset.payload_offset > 0)
                && (hdr->caplen
                        > hdr->extended_hdr.parsed_pkt.offset.payload_offset)) {
            char *payload =
                    (char *) &(skb->data[hdr->extended_hdr.parsed_pkt.offset.payload_offset /* -displ */]);
            int rc = 0, payload_len = hdr->caplen
                    - hdr->extended_hdr.parsed_pkt.offset.payload_offset
                    - displ;
            ...
            if (payload_len > 0) {
                ...
                for (i = 0; (i < MAX_NUM_PATTERN) && (rule->pattern[i] != NULL);
                    ...
                    rc = (textsearch_find_continuous(rule->pattern[i], &state,
    ...
    if ((rule->rule.extended_fields.filter_plugin_id > 0)
            ...
            && (plugin_registration[rule->rule.extended_fields.filter_plugin_id]
                    != NULL)
            && (plugin_registration[rule->rule.extended_fields.filter_plugin_id]->pfring_plugin_filter_skb
                    != NULL)) {
        int rc;
            ...
            if (unlikely(enable_debug))
                printk(
                        ...
                        *last_matched_plugin,
                        parse_memory_buffer[rule->rule.extended_fields.filter_plugin_id],
                        parse_memory_buffer[rule->rule.extended_fields.filter_plugin_id] ?
                                parse_memory_buffer[rule->rule.extended_fields.filter_plugin_id]->mem_len :
                                0);
        }
    }
    ...
    if ((rule->rule.plugin_action.plugin_id != NO_PLUGIN_ID)
            ...
            && (plugin_registration[rule->rule.plugin_action.plugin_id]->pfring_plugin_handle_skb
                    != NULL)) {
        int rc;
    ...
    if (unlikely(enable_debug)) {
        printk(
                "[PF_RING] MATCH: %s(vlan=%u, proto=%u, sip=%u, sport=%u, dip=%u, dport=%u)\n"
                        "          [rule(vlan=%u, proto=%u, ip=%u:%u, port=%u:%u-%u:%u)(behaviour=%d)]\n",
                __FUNCTION__, hdr->extended_hdr.parsed_pkt.vlan_id,
                hdr->extended_hdr.parsed_pkt.l3_proto,
                hdr->extended_hdr.parsed_pkt.ipv4_src,
                hdr->extended_hdr.parsed_pkt.l4_src_port,
                ...
                rule->rule.core_fields.sport_high,
                rule->rule.core_fields.dport_low,
                rule->rule.core_fields.dport_high, *behaviour);
    }
...
static inline void set_skb_time(struct sk_buff *skb, struct pfring_pkthdr *hdr)
{
...
#if(LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30))
    {
        ...
        hdr->extended_hdr.timestamp_ns = ktime_to_ns(
...
static inline int copy_data_to_ring(struct sk_buff *skb,
        struct pf_ring_socket *pfr, struct pfring_pkthdr *hdr, int displ,
        int offset, void *plugin_mem, void *raw_data, uint raw_data_len,
        int *clone_id)
{
    u_char *ring_bucket;
    u_int64_t off;
    u_short do_lock =
            ((enable_tx_capture && pfr->direction != rx_only_direction)
                    ...
                    || (pfr->channel_id_mask == RING_ANY_CHANNEL
                            && get_num_rx_queues(skb->dev) > 1)
                    || (pfr->rehash_rss != NULL
                            && get_num_rx_queues(skb->dev) > 1)
                    || (pfr->num_bound_devices > 1) || (pfr->cluster_id != 0));
    ...
    if (skb != NULL) {
        ...
        if (hdr->caplen > 0) {
            ...
            && (vlan_tci != 0)
                ...
                hdr->caplen = min_val(pfr->bucket_len - offset,
                        hdr->caplen + sizeof(struct eth_vlan_hdr));

                skb_copy_bits(skb, -displ,
                        &ring_bucket[pfr->slot_header_len + offset], displ);
                b =
                        (u_int16_t*) &ring_bucket[pfr->slot_header_len + offset
                                + 12];
                b[0] = ntohs(ETH_P_8021Q), b[1] = ntohs(vlan_tci), b[2] =
                        v->h_vlan_proto;
                if (skb_copy_bits(skb, 0,
                        &ring_bucket[pfr->slot_header_len + offset + 18],
                        (int) hdr->caplen - 18) < 0)
                    printk("[PF_RING] FAULT [skb->len=%u][caplen=%u]\n",
                            skb->len, hdr->caplen - 20);
            } else {
                skb_copy_bits(skb, -displ,
                        &ring_bucket[pfr->slot_header_len + offset],
                        (int) hdr->caplen);
            }
        } else {
            if (hdr->extended_hdr.parsed_header_len >= pfr->bucket_len) {
                ...
                if (!print_once) {
                    printk(
                            ...
                            pfr->bucket_len,
                            hdr->extended_hdr.parsed_header_len);
                    print_once = 1;
                }
            }
        }
        ...
        if (pfr->tx.enable_tx_with_bounce
                && (pfr->header_len == long_pkt_header) && (skb != NULL)
    ...
    /* Set Magic value */
    memset(&ring_bucket[pfr->slot_header_len + offset + hdr->caplen],
    RING_MAGIC_VALUE, sizeof(u_int16_t));
...
static inline int copy_raw_data_to_ring(struct pf_ring_socket *pfr,
        struct pfring_pkthdr *dummy_hdr, void *raw_data, uint raw_data_len)
{
    return (copy_data_to_ring(NULL, pfr, dummy_hdr, 0, 0, NULL, raw_data,
            raw_data_len, NULL));
}

/* ********************************** */

static inline int add_pkt_to_ring(struct sk_buff *skb, u_int8_t real_skb,
        struct pf_ring_socket *_pfr, struct pfring_pkthdr *hdr, int displ,
        int channel_id, int offset, void *plugin_mem, int *clone_id)
{
    struct pf_ring_socket *pfr =
            (_pfr->master_ring != NULL) ? _pfr->master_ring : _pfr;
    ...
    if (real_skb)
        return (copy_data_to_ring(skb, pfr, hdr, displ, offset, plugin_mem,
        NULL, 0, clone_id));
    else
...
static int add_packet_to_ring(struct pf_ring_socket *pfr, u_int8_t real_skb,
        struct pfring_pkthdr *hdr, struct sk_buff *skb, int displ,
...
static int add_raw_packet_to_ring(struct pf_ring_socket *pfr,
        struct pfring_pkthdr *hdr, u_char *data, u_int data_len,
        u_int8_t parse_pkt_first)
{
    int rc;
...
static int add_hdr_to_ring(struct pf_ring_socket *pfr, u_int8_t real_skb,
        struct pfring_pkthdr *hdr)
{
        ...
        u_int8_t freeing_ring)
{
    ...
    if (entry->rule.extended_fields.filter_plugin_id > 0) {
        if (plugin_registration[entry->rule.extended_fields.filter_plugin_id]->pfring_plugin_register)
            plugin_registration[entry->rule.extended_fields.filter_plugin_id]->pfring_plugin_register(
                    0);
    }

    if (entry->rule.plugin_action.plugin_id > 0) {
        if (plugin_registration[entry->rule.plugin_action.plugin_id]->pfring_plugin_register)
            plugin_registration[entry->rule.plugin_action.plugin_id]->pfring_plugin_register(
                    0);
    }
}
...
static void free_sw_filtering_hash_bucket(sw_filtering_hash_bucket * bucket)
{
    ...
    if (bucket->rule.plugin_action.plugin_id > 0) {
        if (plugin_registration[bucket->rule.plugin_action.plugin_id]->pfring_plugin_register)
            plugin_registration[bucket->rule.plugin_action.plugin_id]->pfring_plugin_register(
                    0);
    }
}
...
static int handle_sw_filtering_hash_bucket(struct pf_ring_socket *pfr,
        sw_filtering_hash_bucket * rule, u_char add_rule)
{
    int rc = -1;
    ...
    if (add_rule) {
        ...
        if (rule->rule.plugin_action.plugin_id != NO_PLUGIN_ID) {
            ...
            if (rule->rule.plugin_action.plugin_id >= MAX_PLUGIN_ID)
                ret = -EFAULT;
            else if (plugin_registration[rule->rule.plugin_action.plugin_id]
                    == NULL)
                ret = -EFAULT;
        ...
        if (rule->rule.reflector_device_name[0] != '\0') {
            if ((pfr->ring_netdev->dev != NULL)
                    ...
                    && (strcmp(rule->rule.reflector_device_name,
                            pfr->ring_netdev->dev->name) == 0)) {
                if (unlikely(enable_debug))
                    printk(
                            "[PF_RING] You cannot use as reflection device the same device on "
        ...
        if (pfr->sw_filtering_hash == NULL) {
            pfr->sw_filtering_hash = (sw_filtering_hash_bucket **) kcalloc(
                    perfect_rules_hash_size, sizeof(sw_filtering_hash_bucket *),
                    GFP_ATOMIC);
    ...
    if (pfr->sw_filtering_hash[hash_value] == NULL) {
        ...
        sw_filtering_hash_bucket *prev = NULL, *bucket =
                pfr->sw_filtering_hash[hash_value];
        ...
        while (bucket != NULL) {
            if (hash_filtering_rule_match(&bucket->rule, &rule->rule)) {
                if (add_rule) {
                    if (unlikely(enable_debug))
                        printk(
                                "[PF_RING] Duplicate found while adding rule: discarded\n");
                    return (-EEXIST);
                } else {
                    ...
                    if (unlikely(enable_debug))
                        printk(
                                "[PF_RING] %s() found a bucket to delete: removing it\n",
                                __FUNCTION__);
                    if (prev == NULL)
        ...
        if (add_rule) {
            ...
            if (unlikely(enable_debug))
                printk(
                        "[PF_RING] %s() no duplicate rule found: adding the rule\n",
                        __FUNCTION__);
    ...
    if (add_rule && rc == 0) {
        ...
        if (rule->rule.plugin_action.plugin_id > 0) {
            if (plugin_registration[rule->rule.plugin_action.plugin_id]->pfring_plugin_register)
                plugin_registration[rule->rule.plugin_action.plugin_id]->pfring_plugin_register(
                        1);
        }
    }
...
static int add_sw_filtering_rule_element(struct pf_ring_socket *pfr,
        sw_filtering_rule_element *rule)
{
    ...
    if (rule->rule.reflector_device_name[0] != '\0') {
        if ((pfr->ring_netdev->dev != NULL)
            ...
            if (unlikely(enable_debug))
                printk(
                        "[PF_RING] You cannot use as reflection device the same device on which this ring is bound\n");
            return (-EFAULT);
        }
    ...
    if (rule->rule.extended_fields.filter_plugin_id > 0) {
        if (plugin_registration[rule->rule.extended_fields.filter_plugin_id]->pfring_plugin_register)
            plugin_registration[rule->rule.extended_fields.filter_plugin_id]->pfring_plugin_register(
                    1);
    }
...
static int remove_sw_filtering_rule_element(struct pf_ring_socket *pfr,
        u_int16_t rule_id)
{
    int rule_found = 0;
    struct list_head *ptr, *tmp_ptr;
    ...
    list_for_each_safe(ptr, tmp_ptr, &pfr->sw_filtering_rules)
    {
        ...
        if (entry->rule.rule_id == rule_id) {
            ...
            if (unlikely(enable_debug))
                printk(
                        "[PF_RING] SO_REMOVE_FILTERING_RULE: rule %d has been removed\n",
                        rule_id);
            rule_found = 1;
            break;
        }
...
int check_perfect_rules(struct sk_buff *skb, struct pf_ring_socket *pfr,
        struct pfring_pkthdr *hdr, int *fwd_pkt, u_int8_t *free_parse_mem,
        struct parse_buffer *parse_memory_buffer[MAX_PLUGIN_ID], int displ,
        u_int *last_matched_plugin)
{
    ...
    hash_idx = hash_pkt(hdr->extended_hdr.parsed_pkt.vlan_id,
            hdr->extended_hdr.parsed_pkt.l3_proto,
            hdr->extended_hdr.parsed_pkt.ip_src,
            hdr->extended_hdr.parsed_pkt.ip_dst,
            hdr->extended_hdr.parsed_pkt.l4_src_port,
            hdr->extended_hdr.parsed_pkt.l4_dst_port) % perfect_rules_hash_size;
    ...
    if (hash_found) {
        ...
        if ((hash_bucket->rule.plugin_action.plugin_id != NO_PLUGIN_ID)
                ...
                && (plugin_registration[hash_bucket->rule.plugin_action.plugin_id]
                        != NULL)
                && (plugin_registration[hash_bucket->rule.plugin_action.plugin_id]->pfring_plugin_handle_skb
                        != NULL)) {
            plugin_registration[hash_bucket->rule.plugin_action.plugin_id]->pfring_plugin_handle_skb(
                    pfr, NULL, hash_bucket, hdr, skb, displ, 0, /* no plugin */
        ...
        case bounce_packet_and_stop_rule_evaluation:
            *fwd_pkt = 0;
            reflect_packet(skb, pfr, hash_bucket->rule.internals.reflector_dev,
                    displ, behaviour, 1);
            break;
...
int check_wildcard_rules(struct sk_buff *skb, struct pf_ring_socket *pfr,
        struct pfring_pkthdr *hdr, int *fwd_pkt, u_int8_t *free_parse_mem,
        struct parse_buffer *parse_memory_buffer[MAX_PLUGIN_ID], int displ,
        u_int *last_matched_plugin)
{
    ...
    list_for_each_safe(ptr, tmp_ptr, &pfr->sw_filtering_rules)
    {
        ...
        if (match_filtering_rule(pfr, entry, hdr, skb, displ,
                parse_memory_buffer, free_parse_mem, last_matched_plugin,
                &behaviour)) {
            if (unlikely(enable_debug))
            ...
            hdr->extended_hdr.parsed_pkt.last_matched_rule_id =
                    entry->rule.rule_id;
            ...
            if (behaviour == forward_packet_and_stop_rule_evaluation) {
                ...
                *fwd_pkt = 1;
                ...
                if (*last_matched_plugin
                        ...
                        && plugin_registration[*last_matched_plugin]->pfring_plugin_add_rule
                                != NULL) {
                    ...
                    list_for_each_safe(ptr, tmp_ptr, &pfr->sw_filtering_rules)
                    {
                        sw_filtering_rule_element *tmp_entry = list_entry(ptr,
                                sw_filtering_rule_element, list);
                        if (tmp_entry->rule.rule_id == free_rule_element_id)
                            free_rule_element_id++;
                        else
                            break; /* we found an hole */
                    }
                    ...
                    if (plugin_registration[*last_matched_plugin] != NULL
                        ...
                        rc =
                                plugin_registration[*last_matched_plugin]->pfring_plugin_add_rule(
                                        entry, hdr, free_rule_element_id,
                                        &rule_element, &hash_bucket,
                                        *last_matched_plugin,
                        ...
                        if (rc == 0) {
                            ...
                            if (hash_bucket != NULL) {
                                rc = handle_sw_filtering_hash_bucket(pfr,
                                        hash_bucket,
                            ...
                            if (rule_element != NULL) {
                                rc = add_sw_filtering_rule_element(pfr,
                                        rule_element);
                    ...
                    hash_bucket = (sw_filtering_hash_bucket *) kcalloc(1,
                            sizeof(sw_filtering_hash_bucket), GFP_ATOMIC);
                    ...
                    if (hash_bucket != NULL) {
                        ...
                        write_lock(&pfr->ring_rules_lock);
                        rc = handle_sw_filtering_hash_bucket(pfr, hash_bucket,
                                1 /* add_rule_from_plugin */);
                        write_unlock(&pfr->ring_rules_lock);
                        ...
                        if (rc != 0) {
                            ...
                            if (unlikely(enable_debug))
                                printk(
                                        ...
                                        ((hash_bucket->rule.host4_peer_a >> 24)
                                                & 0xff),
                                        ((hash_bucket->rule.host4_peer_a >> 16)
                                                & 0xff),
                                        ((hash_bucket->rule.host4_peer_a >> 8)
                                                & 0xff),
                                        ((hash_bucket->rule.host4_peer_a >> 0)
                                                & 0xff),
                                        hash_bucket->rule.port_peer_a,
                                        ((hash_bucket->rule.host4_peer_b >> 24)
                                                & 0xff),
                                        ((hash_bucket->rule.host4_peer_b >> 16)
                                                & 0xff),
                                        ((hash_bucket->rule.host4_peer_b >> 8)
                                                & 0xff),
                                        ((hash_bucket->rule.host4_peer_b >> 0)
                                                & 0xff),
                                        hash_bucket->rule.port_peer_b,
                                        pfr->num_sw_filtering_hash);
                        }
                ...
                break;
                    ...
                    == forward_packet_del_rule_and_stop_rule_evaluation) {
                ...
                *fwd_pkt = 1;
                ...
                if (*last_matched_plugin
                        && plugin_registration[*last_matched_plugin] != NULL
                        && plugin_registration[*last_matched_plugin]->pfring_plugin_del_rule
                                != NULL) {
                    ...
                    if (rc > 0) {
                        ...
                        if (rc | 1) {
                            write_lock(&pfr->ring_rules_lock);
                            handle_sw_filtering_hash_bucket(pfr, &hash_bucket,
                                    0 /* del */);
                            write_unlock(&pfr->ring_rules_lock);
                        }
                        ...
                        if (rc | 2) {
                            write_lock(&pfr->ring_rules_lock);
                            remove_sw_filtering_rule_element(pfr,
                                    rule_element_id);
                            write_unlock(&pfr->ring_rules_lock);
                        }
            ...
            if (entry->rule.rule_action
                ...
                *fwd_pkt = 0;
                ...
                printk(
                        "[PF_RING] *** execute_action_and_stop_rule_evaluation\n");
                break;
            } else if (entry->rule.rule_action
                ...
                /* The action has already been performed inside match_filtering_rule()
                 ...
                 will be evaluated */
                    ...
                    || (entry->rule.rule_action
                            == bounce_packet_and_stop_rule_evaluation)) {
                *fwd_pkt = 0;
                reflect_packet(skb, pfr, entry->rule.internals.reflector_dev,
                        displ, entry->rule.rule_action, 1);
                break;
            } else if ((entry->rule.rule_action
 ...
 */
int bpf_filter_skb(struct sk_buff *skb, struct pf_ring_socket *pfr, int displ)
{
    unsigned res = 1;
...
u_int32_t default_rehash_rss_func(struct sk_buff *skb,
        struct pfring_pkthdr *hdr)
{
 ...
 *
 */
static int add_skb_to_ring(struct sk_buff *skb, u_int8_t real_skb,
        struct pf_ring_socket *pfr, struct pfring_pkthdr *hdr, int is_ip_pkt,
        int displ, int channel_id, u_int32_t num_rx_channels, int *clone_id)
{
    int fwd_pkt = 0, rc = 0;
    ...
    if ((!pfring_enabled)
            || ((!pfr->ring_active) && (pfr->master_ring == NULL)))
        return (-1);
    ...
    /* [2.1] Search the hash */
    if (pfr->sw_filtering_hash != NULL) {
        hash_found = check_perfect_rules(skb, pfr, hdr, &fwd_pkt,
                &free_parse_mem, parse_memory_buffer, displ,
                &last_matched_plugin);
        if (hash_found)
            pfr->sw_filtering_hash_match++;
        else
    ...
    if (fwd_pkt) {
        ...
        if (hdr->caplen > 0) {
            ...
            rc = add_pkt_to_ring(skb, real_skb, pfr, hdr, displ, channel_id,
                    offset, mem, clone_id);
        }
    }
        ...
        struct pfring_pkthdr *hdr)
{
    ...
    case cluster_per_flow_2_tuple:
        hash = hash_pkt_header(hdr,
                HASH_PKT_HDR_RECOMPUTE | HASH_PKT_HDR_MASK_PORT
                        | HASH_PKT_HDR_MASK_PROTO | HASH_PKT_HDR_MASK_VLAN);
        break;

    case cluster_per_flow_4_tuple:
        hash = hash_pkt_header(hdr,
                HASH_PKT_HDR_RECOMPUTE | HASH_PKT_HDR_MASK_PROTO
                        | HASH_PKT_HDR_MASK_VLAN);
        break;
    ...
    case cluster_per_flow_tcp_5_tuple:
        if (((hdr->extended_hdr.parsed_pkt.tunnel.tunnel_id == NO_TUNNEL_ID) ?
                ...
                == IPPROTO_TCP)
            hash = hash_pkt_header(hdr,
            HASH_PKT_HDR_RECOMPUTE | HASH_PKT_HDR_MASK_VLAN); /* 5 tuple for TCP */
        else
            hash = hash_pkt_header(hdr,
                    HASH_PKT_HDR_RECOMPUTE | HASH_PKT_HDR_MASK_PORT
                            | HASH_PKT_HDR_MASK_PROTO | HASH_PKT_HDR_MASK_VLAN); /* 2 tuple for non-TCP */
        break;
...
static int register_plugin(struct pfring_plugin_registration *reg)
{
    ...
    if (reg->pfring_plugin_register == NULL)
        printk(
                "[PF_RING] WARNING: plugin %d does not implement handle pfring_plugin_register: please fix it\n",
                reg->plugin_id);

    plugin_registration[reg->plugin_id] = reg;
    plugin_registration_size++;

    max_registered_plugin_id = max_val(max_registered_plugin_id,
            reg->plugin_id);

    printk("[PF_RING] registered plugin [id=%d][max=%d][%p]\n", reg->plugin_id,
            max_registered_plugin_id, plugin_registration[reg->plugin_id]);
...
int unregister_plugin(u_int16_t pfring_plugin_id)
{
    ...
    if (plugin_registration[pfring_plugin_id] == NULL)
        ...
        sk = (struct sock*) lockless_list_get_first(&ring_table,
                &last_list_idx);
...
static inline int is_valid_skb_direction(packet_direction direction,
        u_char recv_packet)
{
    switch (direction) {
    case rx_and_tx_direction:
...
static struct sk_buff* defrag_skb(struct sk_buff *skb, u_int16_t displ,
        struct pfring_pkthdr *hdr, int *defragmented_skb)
{
            ...
            if ((cloned = skb_clone(skb, GFP_ATOMIC)) != NULL) {
                ...
                if (unlikely(enable_debug)) {
                    ...
                    printk(
                            ...
                            iphdr->protocol, offset, ntohs(iphdr->id), ihl, end,
                            hdr->extended_hdr.parsed_pkt.offset.l3_offset
                                    - displ, displ);
                }
...
static int skb_ring_handler(struct sk_buff *skb, u_int8_t recv_packet,
...
/*    u_char buffer[128];
    ...
    *skb_reference_in_use = 0;
    ...
    if ((num_any_rings == 0)
            && (skb->dev && (skb->dev->ifindex < MAX_NUM_IFIDX)
    ...
    if (channel_id > MAX_NUM_RX_CHANNELS)
        channel_id = 0 /* MAX_NUM_RX_CHANNELS */;
...
/*        if (pfr && pfr->rehash_rss != NULL && skb->dev) {
            parse_pkt(skb, real_skb, displ, &hdr, &ip_id);
            channel_id = pfr->rehash_rss(skb, &hdr)
                    % get_num_rx_queues(skb->dev);
            pfr = device_rings[skb->dev->ifindex][channel_id];
        }*/

        //printk("[PF_RING] skb->pkt_hash %llx\n", hdr->extended_hdr.pkt_hash);

        //if (pfr && skb->dev) {
            //parse_pkt(skb, real_skb, displ, &hdr, &ip_id);

            //printk("[PF_RING] ls.a channel prev %d\n", channel_id);
            //printk("[PF_RING] channel after %d\n", channel_id);

/*            data_len = min((u_int16_t) (skb->len + displ),
                    (u_int16_t) sizeof(buffer));
            skb_copy_bits(skb, -displ, buffer, data_len);
            ...
            if (data_len < sizeof(struct ethhdr)) {
                ...
                skb_hash = (int)eh->h_source[ETH_ALEN-1];//default_rehash_rss_func(skb, &hdr);//pfr->rehash_rss(skb, &hdr)
                //num_rxtxqueue = get_num_rx_queues(skb->dev) & 0xFFFFFFFE;*/
                //channel_id = skb_hash & 0x03;
                //pfr = device_rings[skb->dev->ifindex][channel_id];
            //}
        //}
        ...
        if ((pfr != NULL)
            ...
            rc = 1;
            room_available |= copy_data_to_ring(real_skb ? skb : NULL, pfr,
                    &hdr, displ, 0, NULL, NULL, 0, real_skb ? &clone_id : NULL);
        }
    } else {
        ...
        if (enable_ip_defrag) {
            if (real_skb && is_ip_pkt && recv_packet) {
        ...
        while (sk != NULL) {
            ...
            if ((pfr != NULL)
                ...
                room_available |= add_skb_to_ring(skb, real_skb, pfr, &hdr,
                        is_ip_pkt, displ, channel_id, num_rx_channels,
                        &clone_id);
        ...
        cluster_ptr = (ring_cluster_element*) lockless_list_get_first(
                &ring_cluster_list, &last_list_idx);

        /* [2] Check socket clusters */
        while (cluster_ptr != NULL) {
            struct pf_ring_socket *pfr;
            u_short num_cluster_elements = ACCESS_ONCE(
                    cluster_ptr->cluster.num_cluster_elements);

            if (num_cluster_elements > 0) {
                u_short num_iterations;
                int cluster_element_idx;
                int fragment_not_first = hdr.extended_hdr.flags
                        & PKT_FLAGS_IP_FRAG_OFFSET;
                int more_fragments = hdr.extended_hdr.flags
                ...
                if (enable_frag_coherence && fragment_not_first) {
                    if (skb_hash == -1) { /* read hash once */
                        skb_hash = get_fragment_app_id(
                                hdr.extended_hdr.parsed_pkt.ipv4_src,
                                hdr.extended_hdr.parsed_pkt.ipv4_dst, ip_id,
                                more_fragments);
                        if (skb_hash < 0)
                            skb_hash = 0;
                    }
                } else if (!(enable_frag_coherence && first_fragment)
                        || skb_hash == -1) {
                ...
                if (cluster_element_idx >= 0) {
                    ...
                    for (num_iterations = 0;
                        ...
                        skElement =
                                cluster_ptr->cluster.sk[cluster_element_idx];

                        if (skElement != NULL) {
                            pfr = ring_sk(skElement);

                            if ((pfr != NULL) && (pfr->ring_slots != NULL)
                                    && (test_bit(skb->dev->ifindex,
                                            pfr->netdev_mask)
#if(LINUX_VERSION_CODE < KERNEL_VERSION(3,8,0))
                                    ...
                                    && is_valid_skb_direction(pfr->direction,
                                            recv_packet)) {
                                if (check_free_ring_slot(pfr) /* Not full */) {
                                    /* We've found the ring where the packet can be stored */
                                    int old_len = hdr.len, old_caplen =
                                            hdr.caplen; /* Keep old lenght */

                                    room_available |= add_skb_to_ring(skb,
                                            real_skb, pfr, &hdr, is_ip_pkt,
                                ...
                                } else if ((cluster_ptr->cluster.hashing_mode
                                        != cluster_round_robin)
                        ...
                        if (cluster_ptr->cluster.hashing_mode
                                != cluster_round_robin)
                            break;
                        else
                            cluster_element_idx = (cluster_element_idx + 1)
                                    % num_cluster_elements;
                    }
                } else
                    num_cluster_discarded_fragments++;
            ...
            cluster_ptr = (ring_cluster_element*) lockless_list_get_next(
                    &ring_cluster_list, &last_list_idx);
        } /* Clustering */
...
static int buffer_ring_handler(struct net_device *dev, char *data, int len)
{
    ...
    return (skb_ring_handler(&skb, 1, 0 /* fake skb */, &skb_reference_in_use,
...
static int packet_pf_dispatch(struct sk_buff *skb, struct net_device *dev,
...
*/
    ...
    if (skb->pkt_type != PACKET_LOOPBACK
        ...
        rc = skb_ring_handler(skb, (skb->pkt_type == PACKET_OUTGOING) ? 0 : 1,
                1 /* real_skb */, &skb_reference_in_use,
                align_id, //-1 /* unknown: any channel */,
                UNKNOWN_NUM_RX_CHANNELS);
    }
#endif
 ...
 /*    if (!skb_reference_in_use) {
       memset(&hdr, 0, sizeof(hdr));
    	...
    	//if (0 != rc)
        kfree_skb(skb);
        //printk("[PF_RING] kfree_skb, %llx, rc %d\n", (AR_ULONG)skb, rc);
//    }

    return rc;
}

static int packet_rcv_2(struct sk_buff *skb, struct net_device *dev,
        struct packet_type *pt
#if(LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,16))
        , struct net_device *orig_dev
#endif
        )
{
    ...
    if (!skb_reference_in_use) {
        //kfree_skb(skb);
        printk("[PF_RING] 2 kfree_skb, %llx\n", (AR_ULONG)skb);;
    }

    return rc;
}

uint8_t packet_stream_align(struct sk_buff *skb)
{
    uint8_t align_id = 0;
    u_int16_t ip_id = 0;
    int skb_hash = -1;
    ...
    if (skb->dev) {
        ...
        //parse_pkt(skb, 1, displ, &hdr, &ip_id);
        parse_pkt_ip(skb, 1, displ, &hdr, &ip_id);

        skb_hash = default_rehash_rss_func(skb, &hdr);//pfr->rehash_rss(skb, &hdr)
        //num_rxtxqueue = get_num_rx_queues(skb->dev) & 0xFFFFFFFE;*/
        align_id = skb_hash & 0x03;
        //pfr = device_rings[skb->dev->ifindex][channel_id];
    }
...
{
    pf_t_data *pfd;
    /*static */uint8_t t_loop = 0;
    pf_t_buf *pf_tbp;
    int cur_cnt;
    
    if ( !pfring_enabled ) {
    	printk("[PF_RING] %s: pf_en disabled\n", __func__);
    	pfring_dev_pack_run = -1;
    	kfree_skb(skb);
    	pf_tbp->drop_cnt++;
    	return 1;
    }

    //t_loop = packet_stream_align(skb) & 0x03;//(t_loop+1)&0x03;
#if(LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,30))
   	t_loop = skb_get_rx_queue(skb);
#endif
    if (t_loop > 3)
    	t_loop = 0 /* MAX_NUM_RX_CHANNELS */;

    pf_tbp = &pf_td_un[t_loop];
    pf_tbp->align_id = t_loop;

    if ( !pf_tbp->buf_alive ) {
    	//Exiting
    	printk("[PF_RING] %s: pf_t null, drop\n", __func__);
    	pfring_dev_pack_run = -1;
    	kfree_skb(skb);
    	pf_tbp->drop_cnt++;
    	return 1;
    ...
    //read_lock_irq(&pf_tbp->disp_lock);
    cur_cnt = atomic_read(&pf_tbp->cnt);
    //read_unlock_irq(&pf_tbp->disp_lock);

    if ( cur_cnt >= PF_T_SLOT ) {
    	//printk("[PF_RING] %s: pf_t full, drop\n", __func__);
    	kfree_skb(skb);
    	pf_tbp->drop_cnt++;
    	return 1;
    }

    //pf_tbp->dev_pack_state = 1;
    pfring_dev_pack_run = 1;

    spin_lock_irq(&pf_tbp->disp_lock);

    pfd = pf_tbp->pf_tb_data + atomic_read(&pf_tbp->proc);
    pfd->skb = skb;
    pfd->dev = dev;
    pfd->pt = pt;
#if(LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,16))
    pfd->orig_dev = orig_dev;
#endif

/*    printk("[PF_RING]%s: proc, cnt: %d, %d, %llx, skb %llx\n", __func__,
    		atomic_read(&pf_tbp->proc), atomic_read(&pf_tbp->cnt),
    		(AR_ULONG)pfd, (AR_ULONG)pfd->skb);*/

    atomic_inc(&pf_tbp->proc);
    if(atomic_read(&pf_tbp->proc) >= PF_T_SLOT)
    	atomic_set(&pf_tbp->proc, 0);

    //write_lock_irq(&pf_tbp->disp_lock);
    /*while(!spin_trylock_irq(&pf_tbp->disp_lock)){
    	udelay(1);
    }*/

    atomic_inc(&pf_tbp->cnt);
    //write_unlock_irq(&pf_tbp->disp_lock);
    spin_unlock_irq(&pf_tbp->disp_lock);

    //printk("[PF_RING]%s: +++++proc+++++, cnt: %d, %d, loop %d, \n"/*pfd %llx, skb %llx\n"*/, __func__,
    //		atomic_read(&pf_tbp->proc), atomic_read(&pf_tbp->cnt),
    //            t_loop);//, (AR_ULONG)pf_tbp, (AR_ULONG)pfd->skb);

    //kfree_skb(skb);

    pfring_dev_pack_run = 0;
...
void register_device_handler_2(void)
{
    prot_hook_2.func = packet_rcv_2;
    prot_hook_2.type = htons(ETH_P_ALL);
    dev_add_pack(&prot_hook_2);
}
...
void unregister_device_handler_2(void)
{
    dev_remove_pack(&prot_hook_2); /* Remove protocol hook */
}
...
static int ring_proc_virtual_filtering_dev_get_info(struct seq_file *m,
        void *data_not_used)
{
    if (m->private != NULL) {
        virtual_filtering_device_info *info =
                (virtual_filtering_device_info*) m->private;
        char *dev_family = "???";
...
static int ring_proc_virtual_filtering_open(struct inode *inode,
        struct file *file)
{
    return single_open(file, ring_proc_virtual_filtering_dev_get_info,
            PDE_DATA(inode));
}

static const struct file_operations ring_proc_virtual_filtering_fops = {
        .owner = THIS_MODULE, .open = ring_proc_virtual_filtering_open, .read =
                seq_read, .llseek = seq_lseek, .release = single_release, };
        ...
        virtual_filtering_device_info *info)
{
    ...
    list_for_each_safe(ptr, tmp_ptr, &virtual_filtering_devices_list)
    {
        virtual_filtering_device_element *filtering_ptr = list_entry(ptr,
                virtual_filtering_device_element, list);
    ...
    /* Add /proc entry */
    write_lock(&ring_proc_lock);
    elem->info.proc_entry = proc_mkdir(elem->info.device_name,
            ring_proc_dev_dir);
    proc_create_data(PROC_INFO, 0 /* read-only */, elem->info.proc_entry,
            &ring_proc_virtual_filtering_fops /* read */, (void*) &elem->info);
    write_unlock(&ring_proc_lock);
...
static int remove_virtual_filtering_device(struct sock *sock, char *device_name)
{
    ...
    list_for_each_safe(ptr, tmp_ptr, &virtual_filtering_devices_list)
    {
        ...
        if (strcmp(filtering_ptr->info.device_name, device_name) == 0) {
            ...
            remove_proc_entry(filtering_ptr->info.device_name,
                    ring_proc_dev_dir);
            write_unlock(&ring_proc_lock);
...
static unsigned long __get_free_pages_node(int nid, gfp_t gfp_mask,
        unsigned int order)
{
    struct page *page;
        ...
        u_int32_t num_slots, u_int32_t slot_len, u_int32_t chunk_len)
{
    ...
    if ((dma_memory = kcalloc(1, sizeof(struct dma_memory_info), GFP_KERNEL))
            == NULL)
        return NULL;
    ...
    if ((dma_memory->virtual_addr = kcalloc(1,
            sizeof(unsigned long) * dma_memory->num_chunks, GFP_KERNEL)) == NULL) {
        kfree(dma_memory);
        return NULL;
    }

    if ((dma_memory->dma_addr = kcalloc(1,
            sizeof(u_int64_t) * dma_memory->num_slots, GFP_KERNEL)) == NULL) {
    ...
    if (numa_node == -1) {
        if (unlikely(enable_debug))
            printk(
                    "[PF_RING] %s() device node not set, selecting current node\n",
                    __FUNCTION__);
    ...
    if (unlikely(enable_debug))
        printk(
                "[PF_RING] Allocating %d DMA chunks of %d bytes on node %d [slots per chunk=%d]\n",
                dma_memory->num_chunks, dma_memory->chunk_len, numa_node,
                num_slots_per_chunk);

    /* Allocating memory chunks */
    for (i = 0; i < dma_memory->num_chunks; i++) {
        dma_memory->virtual_addr[i] = alloc_contiguous_memory(
                dma_memory->chunk_len, numa_node);

        if (!dma_memory->virtual_addr[i]) {
            printk(
                    "[PF_RING] %s() Warning: no more free memory available! Allocated %d of %d chunks.\n",
                    __FUNCTION__, i + 1, dma_memory->num_chunks);

            dma_memory->num_chunks = i;
            dma_memory->num_slots = dma_memory->num_chunks
                    * num_slots_per_chunk;
            break;
        }
    }
    ...
    for (i = 0; i < dma_memory->num_slots; i++) {
        ...
        if (unlikely(enable_debug))
            printk(
                    "[PF_RING] %s() Mapping DMA slot %d of %d [slot addr=%p][offset=%u]\n",
        ...
        dma_memory->dma_addr[i] = cpu_to_le64(
                pci_map_single(to_pci_dev(dma_memory->hwdev), slot,
                        dma_memory->slot_len, PCI_DMA_BIDIRECTIONAL));
    ...
    if (dma_memory->virtual_addr) {
        for (i = 0; i < dma_memory->num_chunks; i++) {
            if (dma_memory->virtual_addr[i]) {
                ...
                free_contiguous_memory(dma_memory->virtual_addr[i],
                        dma_memory->chunk_len);
            }
        }
        kfree(dma_memory->virtual_addr);
    }
...
static struct dna_cluster* dna_cluster_create(u_int32_t dna_cluster_id,
        u_int32_t num_slots, u_int32_t num_slaves, u_int64_t slave_mem_len,
        u_int64_t master_persistent_mem_len, socket_mode mode,
        u_int32_t options, char *hugepages_dir, struct device *hwdev,
        u_int32_t slot_len, u_int32_t chunk_len, u_int32_t *recovered)
{
    struct list_head *ptr, *tmp_ptr;
    ...
    if (dnac == NULL) {
        ...
        if (!(dnac->options & DNA_CLUSTER_OPT_HUGEPAGES)) {
            if ((dnac->extra_dma_memory = allocate_extra_dma_memory(hwdev,
        ...
        if (num_slaves
                > 0 /* when using direct forwarding num_slaves could also be 0 */) {
            dnac->slave_shared_memory_len = PAGE_ALIGN(slave_mem_len);
            if (!(dnac->options & DNA_CLUSTER_OPT_HUGEPAGES)) {
                shared_mem_size = dnac->slave_shared_memory_len * num_slaves;
                if ((dnac->shared_memory = allocate_shared_memory(
                        &shared_mem_size)) == NULL) {
                    printk(
                            "[PF_RING] %s() ERROR: not enough memory for DNA Cluster shared memory\n",
                            __FUNCTION__);
                    if (!(dnac->options & DNA_CLUSTER_OPT_HUGEPAGES))
                        free_extra_dma_memory(dnac->extra_dma_memory);
                    kfree(dnac);
        ...
        dnac->master_persistent_memory_len = PAGE_ALIGN(
                master_persistent_mem_len);
        shared_mem_size = dnac->master_persistent_memory_len;
        if ((dnac->master_persistent_memory = allocate_shared_memory(
                &shared_mem_size)) == NULL) {
            printk(
                    "[PF_RING] %s() ERROR: not enough memory for DNA Cluster persistent memory\n",
                    __FUNCTION__);
        ...
        dnac->stats =
                (struct dna_cluster_global_stats *) dnac->master_persistent_memory;
        ...
        *recovered = 0;
        ...
        if (dnac->num_slaves != num_slaves
                || (num_slaves > 0
                        && dnac->slave_shared_memory_len
                                != PAGE_ALIGN(slave_mem_len))
                || dnac->master_persistent_memory_len
                        != PAGE_ALIGN(master_persistent_mem_len)
                || dnac->mode != mode
                || (!(dnac->options & DNA_CLUSTER_OPT_HUGEPAGES)
                        && (!dnac->extra_dma_memory
                                || dnac->extra_dma_memory->num_slots
                                        != num_slots))
                || ((dnac->options & DNA_CLUSTER_OPT_HUGEPAGES)
                        && dnac->extra_dma_memory
                        && dnac->extra_dma_memory->num_slots > 0)) {
            dnac = NULL;
            goto unlock;
        }
    ...
    if (dnac != NULL) {
        if (unlikely(enable_debug))
            printk(
                    "[PF_RING] %s(%u) DNA cluster found or created [master: %u]\n",
...
static void dna_cluster_remove(struct dna_cluster *dnac,
        cluster_client_type type, u_int32_t slave_id)
{
    struct list_head *ptr, *tmp_ptr;
    struct dna_cluster *entry;
...
static struct dna_cluster* dna_cluster_attach(u_int32_t dna_cluster_id,
        u_int32_t *slave_id, u_int32_t auto_slave_id,
        ...
        if (entry->id == dna_cluster_id) {
            ...
            if (auto_slave_id) {
                ...
                if (*slave_id >= dnac->num_slaves) {
                    printk(
                            "[PF_RING] %s() slave id is %u, max slave id is %u\n",
            ...
            *slave_mem_len = dnac->slave_shared_memory_len;
            memcpy(hugepages_dir, dnac->hugepages_dir,
            DNA_CLUSTER_MAX_HP_DIR_LEN);
...
static int create_cluster_referee(struct pf_ring_socket *pfr,
        u_int32_t cluster_id, u_int32_t *recovered)
{
    struct list_head *ptr, *tmp_ptr;
    ...
    if (cr == NULL) {
        if (unlikely(enable_debug))
            printk(
                    "[PF_RING] %s: attempting to create a referee for cluster %u\n",
...
static int publish_cluster_object(struct pf_ring_socket *pfr,
        u_int32_t cluster_id, u_int32_t object_type, u_int32_t object_id)
{
    ...
    list_for_each_safe(obj_ptr, obj_tmp_ptr, &cr->objects_list)
    {
        ...
        if (obj_entry->object_type == object_type
                && obj_entry->object_id == object_id) {
        ...
        u_int32_t object_type, u_int32_t object_id, u_int32_t lock_mask)
{
    ...
    if (!cr->master_running) {
        if (unlikely(enable_debug))
            printk(
                    "[PF_RING] %s: cluster %u not running, new locks are not allowed\n",
                    __FUNCTION__, cluster_id);
        goto unlock;
    }
    ...
    list_for_each_safe(obj_ptr, obj_tmp_ptr, &cr->objects_list)
    {
        ...
        if (obj_entry->object_type == object_type
            ...
            if (c_obj->lock_bitmap & lock_mask) {
                if (unlikely(enable_debug))
                    printk(
                            "[PF_RING] %s: trying to lock already-locked features on cluster %u\n",
                            __FUNCTION__, cluster_id);
                goto unlock;
            }
            break;
    ...
    if (c_obj == NULL) {
        if (unlikely(enable_debug))
            printk(
                    "[PF_RING] %s: object %u.%u not in the public list of cluster %u\n",
                    __FUNCTION__, object_type, object_id, cluster_id);
        goto unlock;
    }
...
static int unlock_cluster_object(struct pf_ring_socket *pfr,
        u_int32_t cluster_id, u_int32_t object_type, u_int32_t object_id,
        u_int32_t lock_mask)
{
    struct list_head *ptr, *tmp_ptr;
    ...
    {
        c_obj_entry = list_entry(c_obj_ptr, cluster_object, list);
        if (c_obj_entry->object_type == object_type
                && c_obj_entry->object_id == object_id) {
...
static int ring_release(struct socket *sock)
{
    ...
    if (pfr->kernel_consumer_options)
        ...
        if (pfr->ring_netdev
            ...
            for (i = 0; i < MAX_NUM_RX_CHANNELS; i++) {
                ...
                if (pfr->channel_id_mask & the_bit) {
                    if (device_rings[pfr->ring_netdev->dev->ifindex][i]
                            == pfr) {
                        /*
    ...
    if (pfr->ring_netdev != &none_device_element) {
        ...
        if (pfr->sw_filtering_hash) {
            ...
            for (i = 0; i < perfect_rules_hash_size; i++) {
                if (pfr->sw_filtering_hash[i] != NULL) {
                    sw_filtering_hash_bucket *scan = pfr->sw_filtering_hash[i],
                            *next;
        ...
        if (pfr->num_hw_filtering_rules > 0) {
            ...
            {
                hw_filtering_rule_element *hw_rule = list_entry(ptr,
                        hw_filtering_rule_element, list);
    ...
    if (pfr->v_filtering_dev != NULL) {
        remove_virtual_filtering_device(sk,
                pfr->v_filtering_dev->info.device_name);
    ...
    if (pfr->dna_cluster != NULL)
        dna_cluster_remove(pfr->dna_cluster, pfr->dna_cluster_type,
                pfr->dna_cluster_slave_id);
    ...
    if (pfr->zc_device_entry != NULL) {
        ...
        snprintf(mapping.device_name, sizeof(mapping.device_name), "%s",
...
static int packet_ring_bind(struct sock *sk, char *dev_name)
{
    ...
    if ((dev == &any_device_element) && (!quick_mode)) {
        ...
        if (dev->dev->ifindex < MAX_NUM_IFIDX) {
            ...
            printk(
                    "[PF_RING] INTERNAL ERROR: ifindex %d for %s is > than MAX_NUM_IFIDX\n",
        ...
        struct vm_area_struct *vma)
{
    ...
    if (mem_id >= 100) {
        ...
        if (pfr->zc_dev) {
            if (mem_id < pfr->zc_dev->mem_info.rx.packet_memory_num_chunks) {
                ...
                if ((rc = do_memory_mmap(vma, 0, size,
                        (void *) pfr->zc_dev->rx_packet_memory[mem_id], 0,
                        VM_LOCKED, 1)) < 0)
                    return (rc);

                return (0);
            } else if (mem_id
                    < pfr->zc_dev->mem_info.rx.packet_memory_num_chunks
                ...
                if ((rc = do_memory_mmap(vma, 0, size,
                        (void *) pfr->zc_dev->tx_packet_memory[mem_id], 0,
                        VM_LOCKED, 1)) < 0)
                    return (rc);

                return (0);
            } else if (pfr->extra_dma_memory
                    && mem_id
                            < pfr->zc_dev->mem_info.rx.packet_memory_num_chunks
                                    + pfr->zc_dev->mem_info.tx.packet_memory_num_chunks
                ...
                if ((rc = do_memory_mmap(vma, 0, size,
                        (void *) pfr->extra_dma_memory->virtual_addr[mem_id], 0,
                        VM_LOCKED, 1)) < 0)
                    return (rc);
        ...
        if (pfr->dna_cluster) {
            ...
            if (pfr->dna_cluster->extra_dma_memory == NULL
                    || pfr->dna_cluster->extra_dma_memory->virtual_addr == NULL)
                return (-EINVAL);
            ...
            if (size
                    < pfr->dna_cluster->extra_dma_memory->num_chunks
                            * pfr->dna_cluster->extra_dma_memory->chunk_len)
                return (-EINVAL);

            for (i = 0; i < pfr->dna_cluster->extra_dma_memory->num_chunks;
                    i++) {
                if ((rc =
                        do_memory_mmap(vma,
                                i
    ...
    case 0:
        if (pfr->zc_dev != NULL || pfr->dna_cluster != NULL) {
            printk(
                    "[PF_RING] %s(): trying to map ring memory on DNA/ZC socket\n",
                    __FUNCTION__);
            return (-EINVAL);
        }
        ...
        if (unlikely(enable_debug))
            printk(
                    "[PF_RING] mmap [slot_len=%d][tot_slots=%d] for ring on device %s\n",
                    pfr->slots_info->slot_len, pfr->slots_info->min_num_slots,
                    pfr->ring_netdev->dev->name);

        if ((rc = do_memory_mmap(vma, 0, size, (void *) pfr->ring_memory, 0,
                VM_LOCKED, 0)) < 0)
            return (rc);
    ...
    case 1:
        ...
        if (pfr->zc_dev == NULL) {
            if (unlikely(enable_debug))
                printk("[PF_RING] %s() failed: operation for DNA/ZC only",
                        __FUNCTION__);
            return (-EINVAL);
        }

        if ((rc = do_memory_mmap(vma, 0, size,
                (void *) pfr->zc_dev->rx_descr_packet_memory, 0, VM_LOCKED, 1))
                < 0)
            return (rc);
    ...
    case 2:
        ...
        if ((rc = do_memory_mmap(vma, 0, size,
                (void *) pfr->zc_dev->phys_card_memory, 0, (
    ...
    case 3:
        ...
        if ((rc = do_memory_mmap(vma, 0, size,
                (void *) pfr->zc_dev->tx_descr_packet_memory, 0, VM_LOCKED, 1))
                < 0)
            return (rc);
    ...
    case 4:
        ...
        if (pfr->dna_cluster == NULL
            ...
            if (unlikely(enable_debug))
                printk(
                        "[PF_RING] %s() failed: operation for DNA cluster master only",
                        __FUNCTION__);
            return (-EINVAL);
        }
        ...
        if (pfr->dna_cluster->options & DNA_CLUSTER_OPT_HUGEPAGES) {
            if (unlikely(enable_debug))
                printk(
                        "[PF_RING] %s() failed: trying to allocate kernel memory when using hugepages",
                        __FUNCTION__);
            return (-EINVAL);
        }

        if (size
                > (pfr->dna_cluster->slave_shared_memory_len
                        * pfr->dna_cluster->num_slaves)) {
            if (unlikely(enable_debug))
                printk("[PF_RING] %s() failed: area too large [%ld > %llu]\n",
                        __FUNCTION__, size,
                        pfr->dna_cluster->slave_shared_memory_len
        ...
        if ((rc = do_memory_mmap(vma, 0, size,
                (void *) pfr->dna_cluster->shared_memory, 0, VM_LOCKED, 0)) < 0)
            return (rc);
    ...
    case 5:
        ...
        if (pfr->dna_cluster == NULL
            ...
            if (unlikely(enable_debug))
                printk(
                        "[PF_RING] %s() failed: operation for DNA cluster slave only",
                        __FUNCTION__);
            return (-EINVAL);
        }
        ...
        if ((rc = do_memory_mmap(vma, 0, size,
                (void *) pfr->dna_cluster->shared_memory,
                (pfr->dna_cluster->slave_shared_memory_len / PAGE_SIZE)
                        * pfr->dna_cluster_slave_id, VM_LOCKED, 0)) < 0)
            return (rc);
        ...
        if (size > pfr->dna_cluster->master_persistent_memory_len) {
            if (unlikely(enable_debug))
                printk("[PF_RING] %s() failed: area too large [%ld > %d]\n",
                        __FUNCTION__, size,
                        pfr->dna_cluster->master_persistent_memory_len);
            return (-EINVAL);
        }

        if ((rc = do_memory_mmap(vma, 0, size,
                (void *) pfr->dna_cluster->master_persistent_memory, 0,
                VM_LOCKED, 0)) < 0)
            return (rc);
...
static int pf_ring_inject_packet_to_stack(struct net_device *netdev,
...
unsigned int ring_poll(struct file *file, struct socket *sock,
        poll_table * wait)
{
    struct pf_ring_socket *pfr = ring_sk(sock->sk);
    int rc, mask = 0;
    ...
    if (pfr->zc_dev == NULL) {
        ...
        if (num_queued_pkts(pfr)
                < pfr->poll_num_pkts_watermark /* || pfr->num_poll_calls == 1 */) {
            poll_wait(file, &pfr->ring_slots_waitqueue, wait);
            // smp_mb();
        }
        ...
        if (pfr->zc_dev->wait_packet_function_ptr == NULL) {
            if (unlikely(enable_debug))
                printk(
                        "[PF_RING] wait_packet_function_ptr is NULL: returning to caller\n");

            return (0);
        }

        rc = pfr->zc_dev->wait_packet_function_ptr(pfr->zc_dev->rx_adapter_ptr,
                1);
        ...
        if (rc == 0) {
            ...
            rc = pfr->zc_dev->wait_packet_function_ptr(
                    pfr->zc_dev->rx_adapter_ptr, 0);
...
static int set_master_ring(struct sock *sock, struct pf_ring_socket *pfr,
...
static int add_sock_to_cluster(struct sock *sock, struct pf_ring_socket *pfr,
        struct add_to_cluster *cluster)
{
    ...
    if ((cluster_ptr = kmalloc(sizeof(ring_cluster_element), GFP_KERNEL))
            == NULL) {
        write_unlock(&ring_cluster_lock);
        return (-ENOMEM);
    }
        ...
        zc_dev_mapping *mapping)
{
    ...
    if (unlikely(enable_debug))
        printk("[PF_RING] %s:%d %s %s@%d\n", __FUNCTION__, __LINE__,
                (mapping->operation == remove_device_mapping) ?
                        "remove" : "add", mapping->device_name,
                mapping->channel_id);
    ...
    if (mapping->operation == remove_device_mapping) {
        ...
        list_for_each_safe(ptr, tmp_ptr, &zc_devices_list)
        {
            ...
            if (!strcmp(entry->dev.netdev->name, mapping->device_name)
                ...
                if (!found) {
                    printk(
                            "[PF_RING] %s:%d something got wrong removing %s@%u\n",
                ...
                if (unlikely(enable_debug))
                    printk(
                            "[PF_RING] %s:%d %s@%u removed mapping [num_bound_sockets=%u]\n",
                            __FUNCTION__, __LINE__, mapping->device_name,
                            mapping->channel_id, entry->num_bound_sockets);

                if (pfr->zc_dev != NULL) {
                    pfr->zc_dev->usage_notification(pfr->zc_dev->rx_adapter_ptr,
                            pfr->zc_dev->tx_adapter_ptr, 0 /* unlock */);
                    pfr->zc_device_entry = NULL;
                    pfr->zc_dev = NULL;
                }
            ...
            if (strcmp(entry->dev.netdev->name, mapping->device_name) == 0
                ...
                if (unlikely(enable_debug))
                    printk("[PF_RING] %s:%d %s@%d [num_bound_sockets=%d][%p]\n",
                            __FUNCTION__, __LINE__, entry->dev.netdev->name,
                            mapping->channel_id, entry->num_bound_sockets,
                            entry);
                ...
                if (!found) {
                    printk(
                            "[PF_RING] %s:%d something got wrong adding %s@%u %s\n",
                            __FUNCTION__, __LINE__, mapping->device_name,
                            mapping->channel_id, direction2string(pfr->mode));
                ...
                if (!found) {
                    printk(
                            "[PF_RING] %s:%d add %s@%u %s something got wrong (device not found)\n",
                            __FUNCTION__, __LINE__, mapping->device_name,
                            mapping->channel_id, direction2string(pfr->mode));
                    return -1; /* Something got wrong */
                }
                ...
                if (unlikely(enable_debug))
                    printk(
                            "[PF_RING] %s:%d added mapping %s@%u [num_bound_sockets=%u]\n",
                            __FUNCTION__, __LINE__, mapping->device_name,
                            mapping->channel_id, entry->num_bound_sockets);

                pfr->zc_dev->usage_notification(pfr->zc_dev->rx_adapter_ptr,
                        pfr->zc_dev->tx_adapter_ptr, 1 /* lock */);
    ...
    printk("[PF_RING] %s:%d %s@%u mapping failed or not a ZC/DNA device\n",
            __FUNCTION__, __LINE__, mapping->device_name, mapping->channel_id);
            ...
            list_for_each_safe(ptr, tmp_ptr, &cluster_fragment_hash[i])
            {
                ...
                *frag = list_entry(ptr, struct hash_fragment_node, frag_list);
                ...
                if (frag->expire_jiffies < jiffies) {
                    ...
                    break; /* optimisation: since list is ordered (we are adding to tail) we can skip this collision list */
            }
        } /* for */

        next_fragment_purge_jiffies =
                jiffies + 5 * HZ /* 5 seconds in jiffies */;
    }
}
        ...
        u_int16_t rule_inactivity)
{
    ...
    unsigned long expire_jiffies = jiffies
            - msecs_to_jiffies(1000 * rule_inactivity);
                    ...
                    if (scan->rule.internals.jiffies_last_match < expire_jiffies
                        ...
                        if (unlikely(enable_debug))
                            printk(
                                    ...
                                    scan->rule.port_peer_b, num_purged_rules,
                                    pfr->num_sw_filtering_hash);
    ...
    if (pfr->num_sw_filtering_rules > 0) {
        list_for_each_safe(ptr, tmp_ptr, &pfr->sw_filtering_rules)
        {
            ...
            if (entry->rule.plugin_action.plugin_id > 0
                ...
                rc =
                        plugin_registration[entry->rule.plugin_action.plugin_id]->pfring_plugin_purge_idle(
                                pfr, entry, NULL, rule_inactivity);
            ...
            if ((!entry->rule.locked
                ...
                if (unlikely(enable_debug))
                    printk(
                            ...
                            entry->rule.core_fields.dport_low, num_purged_rules,
                            pfr->num_sw_filtering_rules);
...
static const struct file_operations ring_proc_stats_fops = { .owner =
THIS_MODULE, .open = ring_proc_stats_open, .read = seq_read,
...
int setSocketStats(struct pf_ring_socket *pfr)
{
    ...
    if ((ring_proc_stats_dir != NULL)
        ...
        snprintf(pfr->sock_proc_stats_name, sizeof(pfr->sock_proc_stats_name),
                "%d-%s.%d", pfr->ring_pid, pfr->ring_netdev->dev->name,
                pfr->ring_id);

        if ((entry = proc_create_data(pfr->sock_proc_stats_name, 0 /* ro */,
                ring_proc_stats_dir, &ring_proc_stats_fops, pfr)) == NULL) {
            pfr->sock_proc_stats_name[0] = '\0';
            return (-1);
        }
    }
...
static int ring_getsockopt(struct socket *sock, int level, int optname,
        char __user *optval, int __user *optlen)
{
    ...
    case SO_GET_HASH_FILTERING_RULE_STATS: {
        ...
        if (len >= sizeof(hash_filtering_rule)) {
            ...
            if (pfr->sw_filtering_hash == NULL) {
                printk(
                        "[PF_RING] so_get_hash_filtering_rule_stats(): no hash failure\n");
                return (-EFAULT);
            }

            if (copy_from_user(&rule, optval, sizeof(rule))) {
                printk(
                        "[PF_RING] so_get_hash_filtering_rule_stats: copy_from_user() failure\n");
                return (-EFAULT);
            }

            if (unlikely(enable_debug))
                printk(
                        "[PF_RING] so_get_hash_filtering_rule_stats"
                                "(vlan=%u, proto=%u, sip=%u, sport=%u, dip=%u, dport=%u)\n",
                        rule.vlan_id, rule.proto, rule.host4_peer_a,
                        rule.port_peer_a, rule.host4_peer_b, rule.port_peer_b);

            hash_idx = hash_pkt(rule.vlan_id, rule.proto, rule.host_peer_a,
                    rule.host_peer_b, rule.port_peer_a, rule.port_peer_b)
                    % perfect_rules_hash_size;
            ...
            if (pfr->sw_filtering_hash[hash_idx] != NULL) {
                ...
                if (unlikely(enable_debug))
                    printk(
                            "[PF_RING] so_get_hash_filtering_rule_stats(): bucket=%p\n",
                            bucket);

                while (bucket != NULL) {
                    if (hash_bucket_match_rule(bucket, &rule)) {

                        if ((plugin_registration[rule.plugin_action.plugin_id]
                                == NULL)
                                || (plugin_registration[rule.plugin_action.plugin_id]->pfring_plugin_get_stats
                                        == NULL)) {
                            ...
                            if (buffer == NULL) {
                                printk(
                                        "[PF_RING] so_get_hash_filtering_rule_stats() no memory failure\n");
                                rc = -EFAULT;
                            } else {
                                rc =
                                        plugin_registration[rule.plugin_action.plugin_id]->pfring_plugin_get_stats(
                                                pfr, NULL, bucket, buffer, len);
                                if (rc > 0) {
                                    if (copy_to_user(optval, buffer, rc)) {
                                        printk(
                                                "[PF_RING] copy_to_user() failure\n");
                                        rc = -EFAULT;
                                    }
                                }
                                kfree(buffer);
                ...
                if (unlikely(enable_debug))
                    printk(
                            "[PF_RING] so_get_hash_filtering_rule_stats(): entry not found [hash_idx=%d]\n",
                            hash_idx);
            }
        }
            ...
            if (rule->rule.rule_id == rule_id) {
                ...
                if (buffer == NULL)
                    ...
                    if ((plugin_registration[rule->rule.plugin_action.plugin_id]
                            == NULL)
                            || (plugin_registration[rule->rule.plugin_action.plugin_id]->pfring_plugin_get_stats
                                    == NULL)) {
                        printk(
                                "[PF_RING] Found rule %d but pluginId %d is not registered\n",
                                rule_id, rule->rule.plugin_action.plugin_id);
                        rc = -EFAULT;
                    } else
                        rc =
                                plugin_registration[rule->rule.plugin_action.plugin_id]->pfring_plugin_get_stats(
                                        pfr, rule, NULL, buffer, len);
    ...
    case SO_GET_MAPPED_DNA_DEVICE: {
        ...
        if (copy_to_user(optval, &pfr->zc_dev->mem_info,
                sizeof(zc_memory_info)))
            return (-EFAULT);
    ...
    case SO_GET_EXTRA_DMA_MEMORY: {
        ...
        if (copy_from_user(&slot_len, optval + sizeof(num_slots),
                sizeof(slot_len)))
            return (-EFAULT);

        if (copy_from_user(&chunk_len,
                optval + sizeof(num_slots) + sizeof(slot_len),
                sizeof(chunk_len)))
            return (-EFAULT);
        ...
        if ((pfr->extra_dma_memory = allocate_extra_dma_memory(
                pfr->zc_dev->hwdev, num_slots, slot_len, chunk_len)) == NULL)
            return (-EFAULT);

        if (copy_to_user(optval, pfr->extra_dma_memory->dma_addr,
                (sizeof(u_int64_t) * num_slots))) {
            free_extra_dma_memory(pfr->extra_dma_memory);
            pfr->extra_dma_memory = NULL;
            return (-EFAULT);
        }
    ...
    case SO_GET_NUM_RX_CHANNELS: {
        ...
        if (pfr->ring_netdev == &none_device_element) {
            ...
            if (pfr->ring_netdev->is_zc_device)
                ...
                num_rx_channels = max_val(pfr->num_rx_channels,
                        get_num_rx_queues(pfr->ring_netdev->dev));
        }

        if (unlikely(enable_debug))
            printk(
                    "[PF_RING] --> SO_GET_NUM_RX_CHANNELS[%s]=%d [dna=%d/dns_rx_channels=%d][%p]\n",
                    pfr->ring_netdev->dev->name, num_rx_channels,
                    pfr->ring_netdev->is_zc_device,
                    pfr->ring_netdev->num_zc_dev_rx_queues, pfr->ring_netdev);
    ...
    case SO_GET_BOUND_DEVICE_ADDRESS:
        ...
        if (pfr->zc_dev != NULL) {
            ...
            if (!memcmp(lowest_if_mac, magic_if_mac, ETH_ALEN)) {
                ...
                list_for_each_safe(ptr, tmp_ptr, &ring_aware_device_list)
                {
                    ring_device_element *entry = list_entry(ptr,
                            ring_device_element, device_list);
                    char *eptr;
                    long id = simple_strtol(&entry->dev->name[3], &eptr, 10);

                    if ((lowest_id == -1) || (id < lowest_id)) {
                        lowest_id = id, memcpy(lowest_if_mac,
                                entry->dev->perm_addr, ETH_ALEN);
                    }
                }
                ...
                if (copy_to_user(optval, pfr->ring_netdev->dev->dev_addr,
                ETH_ALEN))
                    return (-EFAULT);
            }
        } else
    ...
    case SO_GET_PKT_HEADER_LEN:
        ...
        if (copy_to_user(optval, &pfr->slot_header_len,
                sizeof(pfr->slot_header_len)))
            return (-EFAULT);
        break;
        ...
        /* Used for testing purposes only */
    {
        ...
        if (len > 0) {
            if (len > loobpack_test_buffer_len)
                return (-EFAULT);

            if (loobpack_test_buffer == NULL) {
                loobpack_test_buffer = kmalloc(loobpack_test_buffer_len,
                        GFP_ATOMIC);
    ...
    case SO_GET_DEVICE_TYPE:
        ...
        if (copy_to_user(optval, &pfr->ring_netdev->device_type,
                sizeof(pfring_device_type)))
            return (-EFAULT);
        break;
    ...
    case SO_GET_APPL_STATS_FILE_NAME: {
        ...
        snprintf(path, sizeof(path) - 1, "/proc/net/pf_ring/stats/%s",
                pfr->sock_proc_stats_name);
        slen = strlen(path);
...
void zc_dev_handler(zc_dev_operation operation, zc_driver_version version,
        mem_ring_info *rx_info, mem_ring_info *tx_info,
        ...
        void *tx_descr_packet_memory, void *phys_card_memory,
        u_int phys_card_memory_len, u_int channel_id, struct net_device *netdev,
        struct device *hwdev, zc_dev_model device_model, u_char *device_address,
        wait_queue_head_t *packet_waitqueue, u_int8_t *interrupt_received,
        ...
        zc_dev_notify dev_notify_function_ptr)
{
    if (unlikely(enable_debug)) {
        printk("[PF_RING] %s(%s@%u [operation=%s])\n", __FUNCTION__,
                ...
                operation == add_device_mapping ?
                        "add_device_mapping" : "remove_device_mapping");
    ...
    if (operation == add_device_mapping) {
        ...
        if (next != NULL) {
            ...
            /* RX */
            if (rx_info != NULL)
                memcpy(&next->dev.mem_info.rx, rx_info,
                        sizeof(next->dev.mem_info.rx));
            if (rx_packet_memory != NULL)
                memcpy(&next->dev.rx_packet_memory, rx_packet_memory,
                        sizeof(next->dev.rx_packet_memory));
            next->dev.rx_descr_packet_memory = rx_descr_packet_memory;

            /* TX */
            if (tx_info != NULL)
                memcpy(&next->dev.mem_info.tx, tx_info,
                        sizeof(next->dev.mem_info.tx));
            if (tx_packet_memory != NULL)
                memcpy(&next->dev.tx_packet_memory, tx_packet_memory,
                        sizeof(next->dev.tx_packet_memory));
            ...
            /* We now have to update the device list */
            {
                ...
                list_for_each_safe(ptr, tmp_ptr, &ring_aware_device_list)
                {
                    ...
                    if (strcmp(dev_ptr->device_name, netdev->name) == 0) {
                        ...
                        dev_ptr->num_zc_dev_rx_queues = max_val(
                                dev_ptr->num_zc_dev_rx_queues, channel_id + 1);
                        ...
                        if (unlikely(enable_debug))
                            printk(
                                    "[PF_RING] ==>> Updating DNA %s [num_zc_dev_rx_queues=%d][%p]\n",
                                    dev_ptr->device_name,
                                    dev_ptr->num_zc_dev_rx_queues, dev_ptr);
                        break;
                    }
                }
            }
        ...
        list_for_each_safe(ptr, tmp_ptr, &zc_devices_list)
        {
            ...
            if ((entry->dev.netdev == netdev)
                ...
                if (found) {
                    printk(
                            "[PF_RING] Trying to unload a ZC driver while using the device!!\n");
    ...
    if (unlikely(enable_debug))
        printk("[PF_RING] %s(%s): [zc_devices_list_size=%d]\n", __FUNCTION__,
                netdev->name, zc_devices_list_size);
}
...
.connect = sock_no_connect, .socketpair = sock_no_socketpair, .accept =
        sock_no_accept, .getname = sock_no_getname, .listen = sock_no_listen,
        .shutdown = sock_no_shutdown, .sendpage = sock_no_sendpage,

        /* Now the operations that really occur. */
        .release = ring_release, .bind = ring_bind, .mmap = ring_mmap, .poll =
                ring_poll, .setsockopt = ring_setsockopt, .getsockopt =
                ring_getsockopt, .ioctl = ring_ioctl, .recvmsg = ring_recvmsg,
        .sendmsg = ring_sendmsg, };

/* ************************************ */

static struct net_proto_family ring_family_ops = { .family = PF_RING, .create =
        ring_create, .owner = THIS_MODULE, };
...
static struct pfring_hooks ring_hooks = { .magic = PF_RING, .ring_handler =
        skb_ring_handler, .buffer_ring_handler = buffer_ring_handler,
        .buffer_add_hdr_to_ring = add_hdr_to_ring, .pfring_registration =
                register_plugin, .pfring_unregistration = unregister_plugin,
    ...
    list_for_each_safe(ptr, tmp_ptr, &ring_aware_device_list)
    {
        ...
        if (dev_ptr->dev == dev) {
            if (dev_ptr->proc_entry != NULL) {
                ...
                remove_proc_entry(
                /*dev_ptr->proc_entry->name*/dev_ptr->device_name,
                        ring_proc_dev_dir);
...
static const struct file_operations ring_proc_dev_fops = { .owner = THIS_MODULE,
        .open = ring_proc_dev_open, .read = seq_read, .llseek = seq_lseek,
        .release = single_release, };
...
int add_device_to_ring_list(struct net_device *dev)
{
    ...
    proc_create_data(PROC_INFO, 0 /* read-only */, dev_ptr->proc_entry,
            &ring_proc_dev_fops /* read */, dev_ptr);

#if(LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,31))
    /* Dirty trick to fix at some point used to discover Intel 82599 interfaces: FIXME */
    if ((dev_ptr->dev->ethtool_ops != NULL)
            && (dev_ptr->dev->ethtool_ops->set_rxnfc != NULL)) {
        struct ethtool_rxnfc cmd;
        int rc;
        ...
        if (rc == RING_MAGIC_VALUE) {
            ...
            /* Setup handlers */
            dev_ptr->hw_filters.filter_handlers.five_tuple_handler =
                    i82599_generic_handler;
            dev_ptr->hw_filters.filter_handlers.perfect_filter_handler =
                    i82599_generic_handler;
            ...
            if (unlikely(enable_debug))
                printk(
                        "[PF_RING] Device %s does NOT support hardware packet filtering [1]\n",
                        dev->name);
        }
    } else {
        if (unlikely(enable_debug))
            printk(
                    "[PF_RING] Device %s does NOT support hardware packet filtering [2]\n",
                    dev->name);
    }
#endif
...
int pf_ring_inject_packet_to_ring(int if_index, int channel_id, u_char *data,
        int data_len, struct pfring_pkthdr *hdr)
{
    struct sock* sk = NULL;
    u_int32_t last_list_idx;
    struct pf_ring_socket *pfr;
    ...
    if (quick_mode) {
        if (if_index < MAX_NUM_IFIDX && channel_id < MAX_NUM_RX_CHANNELS
                && device_rings[if_index][channel_id] != NULL)
            /* 0  == success, -1 == no room available */
            rc = add_raw_packet_to_ring(device_rings[if_index][channel_id], hdr,
                    data, data_len, 0);
    } else {
        sk = (struct sock*) lockless_list_get_first(&ring_table,
                &last_list_idx);
    ...
    if (unlikely(enable_debug) && rc == -2)
        printk(
                "[PF_RING] %s() Error: no ring found for if_index=%d, channel_id=%d\n",
...
static int ring_notifier(struct notifier_block *this, unsigned long msg,
        void *data)
{
    ...
    if (dev != NULL) {
        if (unlikely(enable_debug)) {
            ...
            case NETDEV_REGISTER:
                what = "NETDEV_REGISTER";
                break;
            case NETDEV_UNREGISTER:
                what = "NETDEV_UNREGISTER";
                break;
            case NETDEV_CHANGE:
                what = "NETDEV_CHANGE";
                break;
            case NETDEV_CHANGEADDR:
                what = "NETDEV_CHANGEADDR";
                break;
            case NETDEV_CHANGENAME:
                what = "NETDEV_CHANGENAME";
                break;
            default:
        ...
        if ((dev->type != ARPHRD_ETHER) /* Ethernet */
        /* Wifi */
        && (dev->type != ARPHRD_IEEE80211)
            ...
            if (unlikely(enable_debug))
                printk(
                        "[PF_RING] packet_notifier(%s): skipping non ethernet device\n",
                        dev->name);
            return NOTIFY_DONE;
        }
        ...
        if (dev->ifindex >= MAX_NUM_IFIDX) {
            if (unlikely(enable_debug))
                printk(
                        "[PF_RING] packet_notifier(%s): interface index %d > max index %d\n",
        ...
        case NETDEV_REGISTER:
            if (unlikely(enable_debug))
                printk(
                        "[PF_RING] packet_notifier(%s) [REGISTER][pfring_ptr=%p][hook=%p]\n",
            ...
            list_for_each_safe(ptr, tmp_ptr, &ring_aware_device_list)
            {
                ring_device_element *dev_ptr = list_entry(ptr,
                        ring_device_element, device_list);
                if (dev_ptr->dev != dev
                        && strcmp(dev_ptr->dev->name, dev->name) == 0) {
                    printk(
                            "[PF_RING] WARNING: multiple devices with the same name\n");
        ...
        case NETDEV_UNREGISTER:
            if (unlikely(enable_debug))
                printk(
                        "[PF_RING] packet_notifier(%s) [UNREGISTER][pfring_ptr=%p]\n",
        ...
        case NETDEV_CHANGENAME: /* Rename interface ethX -> ethY */
            ...
            list_for_each_safe(ptr, tmp_ptr, &ring_aware_device_list)
            {
                ...
                if (dev_ptr->dev != dev
                        && strcmp(dev_ptr->dev->name, dev->name) == 0) {
                    printk(
                            "[PF_RING] WARNING: multiple devices with the same name (name clash during name change %s -> %s)\n",
            ...
            list_for_each_safe(ptr, tmp_ptr, &ring_aware_device_list)
            {
                ...
                if (dev_ptr->dev == dev) {
                    if (unlikely(enable_debug))
                        printk(
                                "[PF_RING] ==>> FOUND device change name %s -> %s\n",
                    ...
                    if (!if_name_clash) { /* do not add in case of name clash */
                        ...
                        dev_ptr->proc_entry = proc_mkdir(dev_ptr->dev->name,
                                ring_proc_dev_dir);
        ...
        default:
            if (unlikely(enable_debug))
                printk(
                        "[PF_RING] packet_notifier(%s): unhandled message [msg=%lu][pfring_ptr=%p]\n",
...
int pf_dispatch_t(void *pdata)
{
    int var = 0;
    pf_t_data *pfd;
    pf_t_buf *pf_tbd = (pf_t_buf*)pdata;
    int cur_cnt;

    printk("[PF_RING] IN THREAD %d  FUNCTION %s\n", pf_tbd->tid, __func__);
    pf_tbd->state = 1;

    while( pfring_enabled && pf_tbd->buf_alive ){
        //schedule();
    	//msleep(1);
    	//continue;
    	...
    	//read_lock_irq(&pf_tbd->disp_lock);
    	cur_cnt = atomic_read(&pf_tbd->cnt);
    	//read_unlock_irq(&pf_tbd->disp_lock);

        if ( 0 == cur_cnt ) {
            //printk("[PF_RING] %s empty pf_t, sleep\n", __func__);
            //msleep(1);
            pf_tbd->sleep_cnt++;
            //udelay(1);
            //sched_yield();
            //yield();
            if ( (pf_tbd->sleep_cnt & 0xFFFF) == 0xFFFF ) {
            	msleep(1);
            	pf_tbd->sleep_cnt_r++;
            }
            continue;
        ...
        pfd = pf_tbd->pf_tb_data + atomic_read(&pf_tbd->coms);

#if(LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,16))
        packet_pf_dispatch(pfd->skb, pfd->dev, pfd->pt, pfd->orig_dev, pf_tbd->align_id);
#else
        packet_pf_dispatch(pfd->skb, pfd->dev, pfd->pt, pf_tbd->align_id);
#endif

        /*printk("[PF_RING]%s: coms, cnt: %d, %d, %llx\n", __func__,
        		atomic_read(&pf_tbd->coms), atomic_read(&pf_tbd->cnt),
        		(AR_ULONG)pfd);*/

        spin_lock_irq(&pf_tbd->disp_lock);

        /*atomic_inc(&pf_tbd->coms);
        if(atomic_read(&pf_tbd->coms) >= PF_T_SLOT)
        	atomic_set(&pf_tbd->coms, 0);*/
        pf_tbd->coms.counter = (pf_tbd->coms.counter+1) & PF_T_MASK;

        //write_lock_irq(&pf_tbd->disp_lock);
        /*while(!spin_trylock_irq(&pf_tbd->disp_lock)){
        	udelay(1);
        }*/

        atomic_dec(&pf_tbd->cnt);
        //write_unlock_irq(&pf_tbd->disp_lock);
        spin_unlock_irq(&pf_tbd->disp_lock);

        pf_tbd->total_cnt++;

        //printk("[PF_RING]%s: -----coms-----, cnt: %d, %d, tid %d, total %llu\n"/*tbd %llx, skb %llx\n"*/, __func__,
        //		atomic_read(&pf_tbd->coms), atomic_read(&pf_tbd->cnt),
        //                pf_tbd->tid, pf_tbd->total_cnt);//, (AR_ULONG)pf_tbd, (AR_ULONG)pfd->skb);
    ...
    printk("[PF_RING] OUT THREAD %d FUNCTION %s\n", pf_tbd->tid, __func__);

    //atomic_set(&pf_tbd->cnt, 0);
    pf_tbd->state = -1;
    /*do_exit(1);*/
    return var;
}

static int pf_t_init(pf_t_buf *pf_tb)
{
//    return 0;
    char t_name[16];

    pf_tb->pf_tb_data = (pf_t_data*)kmalloc(PF_T_BUF_LEN, GFP_KERNEL);
    if (NULL == pf_tb->pf_tb_data) {
    	printk("[PF_RING] memory is not OK\n");
        return 0;
    }

    pf_tb->buf_alive = 1;

    //rwlock_init(&pf_tb->disp_lock);
    spin_lock_init(&pf_tb->disp_lock);

    snprintf(t_name, sizeof(t_name), "kpf_tpp_%d", pf_tb->tid);
    pf_tb->task = kthread_create(pf_dispatch_t, (void *)pf_tb, t_name);
    if (!IS_ERR(pf_tb->task)) {
        kthread_bind(pf_tb->task, pf_tb->acid);
        printk("[PF_RING] kthread_bind (tid %d, acid %d) \n", pf_tb->tid, pf_tb->acid);
        wake_up_process(pf_tb->task);
    }
    else {
        printk("[PF_RING] kthread_create (tid %d)  failed\n", pf_tb->tid);
        return 0;
    }

    printk("[PF_RING] %s tid %d: task %llx, task->comm %s\n", __func__,
                  pf_tb->tid, (AR_ULONG)pf_tb->task, pf_tb->task->comm);
    return 0;
}

static void pf_t_stop(pf_t_buf *pf_tb)
{
    //return;
	/*    if ( !IS_ERR(pf_tb->task) && 1==pf_tb->state ) {
	        kthread_stop(pf_tb->task);
	        printk("[PF_RING] %s, tid %d task %llx, state %d\n", __func__,
	                       pf_tb->tid, (AR_ULONG)pf_tb->task, pf_tb->state);
	    }*/

	pf_tb->buf_alive = 0;

    while( 1 == pf_tb->state ) {// || 0!=atomic_read(&pf_tb->cnt)){
        msleep(10);
        printk("[PF_RING] waiting pf_t stop, tid %d\n", pf_tb->tid);
    }

    while( 1 == pfring_dev_pack_run ) {
        msleep(10);
        printk("[PF_RING] waiting pf_dev stop, tid %d\n", pf_tb->tid);
    }

    printk("[PF_RING] %s, tid %d\n", __func__, pf_tb->tid);
}

static void pf_t_exit(pf_t_buf *pf_tb)
{
	pf_t_data *pfd;
	uint32_t re_cnt = 0;

    while ( atomic_read(&pf_tb->cnt) > 0 )
    {
   	    pfd = pf_tb->pf_tb_data + atomic_read(&pf_tb->coms);
    	kfree_skb(pfd->skb);

    	//spin_lock_irq(&pf_tbd->disp_lock);

    	atomic_inc(&pf_tb->coms);
    	if(atomic_read(&pf_tb->coms) >= PF_T_SLOT)
    		atomic_set(&pf_tb->coms, 0);
    	atomic_dec(&pf_tb->cnt);

    	re_cnt++;
    }

    printk("[PF_RING] %s, tid %d, free left skb cnt %d\n", __func__, pf_tb->tid, re_cnt);

    if (NULL != pf_tb->pf_tb_data)
        kfree(pf_tb->pf_tb_data);
}

/* ************************************ */

static struct notifier_block ring_netdev_notifier = { .notifier_call =
        ring_notifier, };
...
static void __exit ring_exit(void)
{
  ...
  //unregister_device_handler_2();

    pfring_enabled = 0;

    pf_t_stop(&pf_td_un[0]);
    pf_t_stop(&pf_td_un[1]);
    pf_t_stop(&pf_td_un[2]);
    pf_t_stop(&pf_td_un[3]);

    pf_t_exit(&pf_td_un[0]);
    pf_t_exit(&pf_td_un[1]);
    pf_t_exit(&pf_td_un[2]);
    pf_t_exit(&pf_td_un[3]);

  list_del(&any_device_element.device_list);
  list_for_each_safe(ptr, tmp_ptr, &ring_aware_device_list){
    ring_device_element *dev_ptr;

    dev_ptr = list_entry(ptr,
        ring_device_element, device_list);hook = (struct pfring_hooks*)dev_ptr->dev->pfring_ptr;
...
if(hook->magic == PF_RING) {
  ...
  if(enable_frag_coherence && num_cluster_fragments > 0) {
    ...
    for(i=0; i<NUM_FRAGMENTS_HASH_SLOTS; i++) {
      list_for_each_safe(ptr, tmp_ptr, &cluster_fragment_hash[i]){
        struct hash_fragment_node *frag = list_entry(ptr,
        struct hash_fragment_node, frag_list);list_del(ptr);
	kfree(frag);
      }
    }
  }
...
static int __init ring_init(void)
{
  ...
  memset(pf_td_un, 0, sizeof(pf_td_un));
  pf_td_un[0].acid = 12;
  pf_td_un[1].acid = 13;
  pf_td_un[2].acid = 14;
  pf_td_un[3].acid = 15;
  ...
  pf_t_init(&pf_td_un[0]);
  pf_t_init(&pf_td_un[1]);
  pf_t_init(&pf_td_un[2]);
  pf_t_init(&pf_td_un[3]);

  register_device_handler();

  //register_device_handler_2();
